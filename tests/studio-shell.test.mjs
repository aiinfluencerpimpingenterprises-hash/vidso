...
test('sidebar uses Create because Chat Mode and Director Mode do not exist', () => {
  ...
  const create = SIDEBAR_SECTIONS.find((s) => s.id === 'create')
  assert.equal(create.items.find((i) => i.id === 'facelessstudio')?.name, 'Faceless Studio')
...
test('home rails and inspiration tabs are config-driven', () => {
  ...
  assert.ok(shell.includes('id="studio-mode-tools"'))
