...
test('placeholder mappings stay honest', () => {
  ...
  assert.ok(html.includes('mcp-feat-ink'))
  assert.ok(html.includes('mcp-feat-stage'))
  assert.ok(html.includes('id="mcp-models"'))
  assert.ok(html.includes('data-mcp-models'))
  assert.ok(html.includes('data-faq-single'))
  assert.ok(html.includes('Which models can the agent use?'))
  assert.ok(html.includes('How do I connect Kimi, Grok Bot, or other agents?'))
  assert.equal((html.match(/<details/g) || []).length, 14)
  assert.ok(css.includes('mcp-feat-ink'))
  assert.ok(css.includes('height:440px'))
  assert.ok(css.includes('height:480px'))
  assert.ok(css.includes('.mcp-ask-list::before'))
  assert.ok(css.includes('.mcp-models-grid'))
  assert.ok(css.includes('rgba(255,255,255,.10)'))
  assert.ok(!css.includes('.mcp-ask-media .mcp-blank{\n  display:block;width:100%;border:2px solid #FE0C30'))
  ...
  assert.ok(page.includes('data-mcp-feat-ink'))
  assert.match(readFileSync(new URL('../lib/landing-page.js', import.meta.url), 'utf8'), /openId/)
  assert.ok(css.includes('.mcp-mock [hidden]{display:none!important}'))
  assert.ok(css.includes('align-items:stretch'))
  assert.ok(css.includes('.mcp-ask-list{display:flex;flex-direction:column;gap:4px;position:relative;z-index:2;min-height:0}'))
