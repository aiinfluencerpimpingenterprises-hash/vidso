...
test('format and ugc placeholders use slug filenames', () => {
  ...
  assert.ok(FORMAT_CARDS.filter((c) => !['long-form', 'shorts', 'ads', 'ugc', 'explainers', 'product-videos', 'talking-head'].includes(c.slug)).every((c) => c.src.endsWith('/format-card-' + c.slug + '.mp4')))
