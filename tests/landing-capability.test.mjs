...
test('format and ugc placeholders use slug filenames', () => {
  assert.equal(FORMAT_CARDS.length, 7)
  assert.deepEqual(FORMAT_CARDS.map((c) => c.category), [
    'Long-form', 'Shorts', 'Ads', 'UGC', 'Explainers', 'Product videos', 'Talking head',
  ])
  assert.equal(FORMAT_CARDS.find((c) => c.slug === 'long-form').src, 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/landingpagelongform.mp4')
  ...
  assert.equal(FORMAT_CARDS.find((c) => c.slug === 'product-videos').src, 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/landingpageformatsproduct.mp4')
  assert.equal(FORMAT_CARDS.find((c) => c.slug === 'talking-head').src, 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/landingpageformatstalkinghead.mp4')
  assert.equal(FORMAT_CARDS.filter((c) => c.slug === 'listicles' || c.slug === 'documentary').length, 0)
  assert.deepEqual(PLACEHOLDER_FILES.formats.slice(0, 4), ['landingpagelongform.mp4', 'landingpageformatsshort.mp4', 'landingpageformatsad.mp4', 'landingpageformatsugc.mp4'])
