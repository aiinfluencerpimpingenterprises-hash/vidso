import { LANDING_R2, MCP_CHAT_MEDIA, MCP_FEATURE_MEDIA, R2 } from './landing-media.js?v=d1f96'
...
if (typeof window !== 'undefined' && !window.__vidsoMcpAudioBound) {
  ...
  document.addEventListener('pointerover', (e) => {
    document.querySelectorAll('[data-mcp-hover-audio] video').forEach((video) => {
...
function muteSlotVideo(video) {
  if (!video) return
  video.muted = true
}

function stopSlotVideo(video) {
  if (!video) return
  muteSlotVideo(video)
...
function unbindHoverAudio(host) {
  if (!host) return
  host.__mcpHoverAudio?.abort()
  host.__mcpHoverAudio = null
  host.classList.remove('is-hover-audio')
}

function bindHoverAudio(host, video) {
  if (!host || !video) return
  unbindHoverAudio(host)
  if (!host.hasAttribute('data-mcp-hover-audio')) return
  const ac = new AbortController()
  const signal = ac.signal
  host.__mcpHoverAudio = ac
  host.classList.add('is-hover-audio')
  const hoveringVideo = (e) => e.target === video || video.contains(e.target)
  const unmute = (e) => {
    if (!audioUnlocked) return
    if (!host.hasAttribute('data-mcp-hover-audio')) return
    if (!host.contains(video) || !document.contains(video)) return
    if (!slotVisible(host)) return
    if (e && !hoveringVideo(e)) return
    document.querySelectorAll('video').forEach((other) => {
      if (other !== video) muteSlotVideo(other)
    })
    video.muted = false
    playSlotVideo(video)
  }
  const mute = () => muteSlotVideo(video)
  video.addEventListener('pointerenter', unmute, { signal })
  video.addEventListener('pointerleave', mute, { signal })
...
export function fillMediaSlot(card, opts = {}) {
  if (!card) return
  ...
  const urls = [R2 + '/' + encoded, LANDING_R2 + '/' + encoded]
  const existing = card.querySelector('img,video')
  if (existing) {
    const cur = existing.getAttribute('src') || existing.currentSrc || ''
    if (cur.includes('/' + encoded)) return
    if (!opts.replace) return
    if (existing.tagName === 'VIDEO') stopSlotVideo(existing)
    unbindHoverAudio(card)
    existing.remove()
  ...
  if (img.complete && img.naturalWidth) card.classList.remove('is-blank')
}
