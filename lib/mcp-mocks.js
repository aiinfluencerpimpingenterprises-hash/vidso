import { LANDING_R2, MCP_CHAT_MEDIA, MCP_FEATURE_MEDIA } from './landing-media.js'
...
export function fillMediaSlot(card) {
  ...
  if (!file) return
  const base = LANDING_R2 + '/' + file
  const poster = ext === 'mp4' ? base + '.jpg' : base + (ext ? '.' + ext : '.jpg')
  const video = ext === 'mp4' ? base + '.mp4' : ''
