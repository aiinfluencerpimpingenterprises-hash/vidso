...
function promoHtml(promo, extra = '') {
  return `<div class="models-mega-promo">
    <div class="models-mega-shot is-blank" data-nav-promo="${esc(promo.src)}"></div>
    <p class="models-mega-cap">${esc(promo.caption)}</p>
    <a class="btn btn-primary models-mega-browse" role="menuitem" href="${esc(promo.browseHref)}">${esc(promo.browse)}</a>
    ${extra}
  </div>`
}
...
function featuresPanelHtml() {
  ...
  return `<div class="tools-mega-inner features-mega-inner">
    <div class="features-mega-grid">
      ${promoHtml({ src: FEATURES_PROMO, caption: FEATURES_PROMO_COPY.caption, browse: FEATURES_PROMO_COPY.browse, browseHref: FEATURES_PROMO_COPY.browseHref })}
...
function probePromo(root) {
  root.querySelectorAll('[data-nav-promo]').forEach((host) => {
    const src = host.getAttribute('data-nav-promo')
    if (!src) return
    fetch(src, { method: 'GET', mode: 'cors', headers: { Range: 'bytes=0-0' } })
