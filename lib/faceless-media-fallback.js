 ...
 * (Vidso's gate implements it). Re-uploading a decoded WAV of a long VO also
 * fails Railway's upload limits, so prefer byte-concat of the original MP3s.
 */
...
export function isUploadFailedError(err) {
  const msg = typeof err === 'string' ? err : String(err?.message || err?.error || '')
  return /upload failed|could not save the merged voiceover|file too large|payload too large|entity too large/i.test(msg)
}

export function isVoiceoverJoinError(err) {
  const msg = typeof err === 'string' ? err : String(err?.message || err?.error || '')
  return isMediaConcatError(msg)
    || isUploadFailedError(msg)
    || /could not download a voiceover chunk|could not join the voiceover|could not read a voiceover/i.test(msg)
}

/** Public CDN URLs for a long voiceover. Never fetch these in the browser (CORS / size). */
export function voiceoverPlaylist(media) {
  const many = (media?.voiceover_urls || []).map(String).filter((u) => /^https?:\/\//i.test(u) || u.startsWith('blob:'))
  if (many.length) return [...new Set(many)]
  const one = String(media?.voiceover_url || '').trim()
  return one ? [one] : []
}

export function playlistOffsetAt(durations, globalTime) {
  const durs = (durations || []).map((d) => Math.max(0, Number(d) || 0))
  let t = Math.max(0, Number(globalTime) || 0)
  for (let i = 0; i < durs.length; i++) {
    const dur = durs[i] || 0
    if (i === durs.length - 1 || t < dur) return { index: i, local: Math.min(t, dur || t) }
    t -= dur
  }
...
/**
 * Join already-encoded voiceover parts by concatenating their bytes.
 * TTS returns MP3; byte-joining keeps the file small enough for Railway upload
 * and avoids decoding 30 minutes of audio into a huge WAV in the browser.
 */
export async function concatMpegUrls(urls, options = {}) {
  ...
  for (const url of list) {
    ...
    if (ct && /audio\//i.test(ct)) mime = ct
    parts.push(await res.arrayBuffer())
  }
  const total = parts.reduce((n, p) => n + p.byteLength, 0)
  return {
    blob: new Blob(parts, { type: mime || 'audio/mpeg' }),
    bytes: total,
    mime: mime || 'audio/mpeg',
    filename: /wav/i.test(mime) ? 'faceless-voiceover.wav' : 'faceless-voiceover.mp3',
  }
}
