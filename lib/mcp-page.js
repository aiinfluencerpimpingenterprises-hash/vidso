...
import { mountAnnounce, mountFaq, mountNavShrink, wrapGlowLabels } from './landing-page.js?v=d1faa'
...
import { CATALOG_CLIP_FILES, bindCatalogClipLoop, catalogClipWindow, mcpLatestModels, modelCardClipSrc, modelCardSrc, modelSlug } from './vidso-models.js?v=d1fa5'
...
function mountMcpModels(root = document) {
  ...
  grid.innerHTML = models.map((model, i) => {
    ...
    return `<div class="mcp-model-tile" data-model="${slug}" style="--i:${i}">
...
function attachMcpClip(host, url, loopSeconds) {
...
function bindMcpModelTile(tile, model) {
  ...
  const loopSeconds = catalogClipWindow(model.id)
  if (CATALOG_CLIP_FILES[model.id] || CATALOG_CLIP_FILES[modelSlug(model.id)]) {
    attachMcpClip(host, clip, loopSeconds)
    return
  }
  probeUrl(clip).then((ok) => { if (ok) attachMcpClip(host, clip, loopSeconds) })
}
