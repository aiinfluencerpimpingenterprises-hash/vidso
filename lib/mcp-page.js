...
import { fillAllMediaSlots, fillMediaSlot, mountAskMock, mountFeatureMocks } from './mcp-mocks.js?v=d1f84'
...
function mountAsk(root = document, askMock) {
  ...
  function paint(next) {
    ...
    if (slot) {
      ...
      if (ext === 'mp4') {
        slot.setAttribute('data-mcp-loop-seconds', '15')
        slot.setAttribute('data-mcp-hover-audio', '')
      } else {
        slot.removeAttribute('data-mcp-loop-seconds')
