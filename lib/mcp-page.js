import { CLAUDE_ICON, MCP_CONNECT_TOOLS, VIDSO_CLI } from './landing-media.js'
...
import { mountLandingReveals, checkHeadingCenters } from './landing-reveal.js?v=d1f32'
import { mountLandingToolsMenu } from './landing-tools-menu.js'
import { fillAllMediaSlots, fillMediaSlot, mountAskMock, mountFeatureMocks } from './mcp-mocks.js'
...
function sizeClientPanes(host, panes) {
  if (!host) return
  const prev = host.style.minHeight
  host.style.minHeight = '0px'
  let tallest = 0
  panes.forEach((pane) => {
    const wasOn = pane.classList.contains('is-on')
    pane.classList.add('is-measure')
    tallest = Math.max(tallest, pane.offsetHeight)
    if (!wasOn) pane.classList.remove('is-measure')
  })
  host.style.minHeight = prev
  if (tallest) host.style.height = tallest + 'px'
}

function mountModeToggle(root = document) {
  const mcpBtn = root.querySelector('[data-mcp-mode="mcp"]')
  const cliBtn = root.querySelector('[data-mcp-mode="cli"]')
  const mcpPanes = root.querySelector('[data-mcp-client-panes]')
  const cliPane = root.querySelector('[data-mcp-cli-pane]')
  if (!mcpBtn || !cliBtn || !mcpPanes || !cliPane) return
  function show(mode) {
    const cli = mode === 'cli'
    mcpBtn.classList.toggle('is-on', !cli)
    cliBtn.classList.toggle('is-on', cli)
    mcpBtn.setAttribute('aria-pressed', cli ? 'false' : 'true')
    cliBtn.setAttribute('aria-pressed', cli ? 'true' : 'false')
    mcpPanes.hidden = cli
    cliPane.hidden = !cli
  }
  mcpBtn.addEventListener('click', () => show('mcp'))
  cliBtn.addEventListener('click', () => show('cli'))
}
...
function fillConnectRows(root = document) {
  ...
  MCP_CONNECT_TOOLS.forEach((name) => {
    ...
    li.innerHTML = '<span class="mcp-check" aria-hidden="true"></span><code></code>'
...
function mountClientTabs(root = document) {
  ...
  const host = root.querySelector('[data-mcp-client-panes]')
  ...
  function show(btn) {
    ...
    sizeClientPanes(host, panes)
  }
  tabs.forEach((tab) => { tab.tabIndex = tab.classList.contains('is-on') ? 0 : -1 })
  panes.forEach((pane) => {
    if (!pane.classList.contains('is-on')) pane.setAttribute('inert', '')
  })
  bindTablist(tabs, show)
  sizeClientPanes(host, panes)
  window.addEventListener('resize', () => sizeClientPanes(host, panes))
}
