...
export function claudeDemoHtml(demo, cards = []) {
  const count = Math.max(4, Number(demo.placeholders) || 8)
    ...
    <div class="claude-demo">
      <div class="mac-window">
        ...
        <div class="mac-body">
          ...
          <div class="claude-ph-grid">${grid}</div>
...
export function toolCardHtml(tool) {
  return `<article class="tool-card">
    ${mediaPlaceholderHtml({ src: tool.image || '', ratio: '16 / 9', className: 'tool-shot', alt: tool.name })}
    <div class="tool-copy">
      <h3>${esc(tool.name)}</h3>
