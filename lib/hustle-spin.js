...
/** Fast burst, then a stepped brake so the last rows tick in rhythm. */
export function easeOutSlot(t) {
  const x = Math.min(1, Math.max(0, t))
  if (x < 0.4) {
    const u = x / 0.4
    return 0.86 * (1 - Math.pow(1 - u, 2))
  }
  if (x < 0.66) {
    const u = (x - 0.4) / 0.26
    return 0.86 + 0.08 * (1 - Math.pow(1 - u, 1.7))
  }
  if (x < 0.86) {
    const u = (x - 0.66) / 0.2
    return 0.94 + 0.045 * u
  ...
  const u = (x - 0.86) / 0.14
  return 0.985 + 0.015 * (1 - Math.pow(1 - u, 2))
}
