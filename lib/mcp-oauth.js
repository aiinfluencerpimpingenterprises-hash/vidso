...
/**
 * Flip to true to hide `/mcp`, the OAuth sidecar, and `.well-known` metadata.
 * Adding ?mcp=1 or `x-vidso-mcp-preview: 1` still reaches the server when archived.
 * VIDSO_MCP_ARCHIVED=1/0 on Vercel overrides this constant.
 */
export const MCP_ARCHIVED = false

export function mcpPreviewOn(req) {
  if (String(req?.headers?.['x-vidso-mcp-preview'] || '') === '1') return true
  const query = String(req?.url || '').split('?')[1] || ''
  return new URLSearchParams(query).get('mcp') === '1'
}

/** True when this request must not reach the MCP server. */
export function mcpArchived(req) {
  const flag = String(process.env.VIDSO_MCP_ARCHIVED ?? '').trim().toLowerCase()
  const archived = flag ? !(flag === '0' || flag === 'false') : MCP_ARCHIVED
  if (!archived) return false
  return !mcpPreviewOn(req)
}
...
<title>Vidso · Claude connector</title>
<style>
...
<div class="card">
  ...
  <ol>
    ...
    <li>Name it <strong>Vidso</strong></li>
    <li>Paste this URL, then Connect and sign in to Vidso</li>
  </ol>
  <pre id="u">${safe}</pre>
  <button type="button" id="c">Copy link</button>
  <p>After Claude connects, ask it to make a long-form video, a thumbnail, or connect YouTube. Channel connect still uses a Google link. You can also manage the account in <a href="/home">Account settings</a>.</p>
</div>
<script>
