...
export function protectedResourceMetadataPath(resourceUrl) {
  try {
    const u = new URL(resourceUrl)
    const path = u.pathname.replace(/\/+$/, '') || '/'
    if (path === '/' ) return '/.well-known/oauth-protected-resource'
    return '/.well-known/oauth-protected-resource' + path
  } catch {
    return '/.well-known/oauth-protected-resource' + MCP_CONNECTOR_PATH
  }
}
