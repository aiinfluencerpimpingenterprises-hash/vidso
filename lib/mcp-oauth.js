...
/**
 * The YouTube MCP server is archived: `/mcp`, its legacy path, the OAuth
 * sidecar, and the `.well-known` metadata all answer 404, so no connector can
 * discover or reach it. The code, routes, and stored connections are untouched.
 *
 * To un-archive: flip this to false. Adding ?mcp=1 to any of those URLs, or
 * sending `x-vidso-mcp-preview: 1`, reaches the live server anyway, so it stays
 * testable on production.
 */
export const MCP_ARCHIVED = true
...
<div class="card">
  ...
  <p>Connect a YouTube channel in Vidso first if you want Claude to list, edit, or upload videos. Open <a href="/home">vidso.pro</a>.</p>
</div>
<script>
