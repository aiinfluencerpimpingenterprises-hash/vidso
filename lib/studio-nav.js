...
export const SIDEBAR_SECTIONS = [
  ...
  },
  {
    id: 'create',
    label: 'Create',
    items: [STUDIO_TOOL_COPY.videogen, STUDIO_TOOL_COPY.imagegen].map((t) => ({ ...navTool(t), name: t.short || t.name, fullName: t.name })),
  },
  {
    id: 'tools',
    label: 'Tools',
...
export function inspireItems(category) {
  ...
  if (cat.id === 'marketing') {
    return MARKETING_CLIPS.map((file, i) => ({
      id: 'marketing-' + (i + 1),
      n: i + 1,
      category: 'marketing',
      title: 'Marketing and Advertising ' + String(i + 1).padStart(2, '0'),
      file,
