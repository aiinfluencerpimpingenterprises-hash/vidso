...
const IMG_CACHE_KEY = 'vidso_img_cache_v1'
...
export function initImageWorkspace(deps) {
  ...
  function readCache() {
    try {
      const all = JSON.parse(localStorage.getItem(IMG_CACHE_KEY) || '{}')
      if (Array.isArray(all)) return all
      const uid = cacheUserKey()
      const direct = all[uid] || all.anon
      if (Array.isArray(direct) && direct.length) return direct
      const extra = Object.values(all).find((v) => Array.isArray(v) && v.length)
      return extra || (Array.isArray(direct) ? direct : [])
    } catch (_) { return [] }
  }
  ...
  function writeCache() {
    try {
      const slim = items
        .filter((it) => !it.pending && !it.error && (it.storage_url || it.url))
        .slice(0, 48)
      const raw = JSON.parse(localStorage.getItem(IMG_CACHE_KEY) || '{}')
      const bag = Array.isArray(raw) ? {} : (raw || {})
      bag[cacheUserKey()] = slim
      bag.anon = slim
      localStorage.setItem(IMG_CACHE_KEY, JSON.stringify(bag))
    } catch (_) {}
  }
  ...
  function closePopovers(except) {
    ...
    document.querySelectorAll('.img-more.is-open').forEach((el) => {
      if (except && el === except) return
      el.classList.remove('is-open')
      el.closest('.img-tile')?.classList.remove('is-menu')
    })
  }
  ...
  function renderGrid() {
    ...
    const hero = filter !== 'favs' && !items.length
  ...
  async function loadPage(reset) {
    ...
    try {
      ...
      if (reset) {
        const locals = items.filter((it) => it.pending || it.error)
        const seen = new Set(next.map((it) => it.id))
        const extras = items.filter((it) => (
          !it.pending && !it.error && (it.storage_url || it.url) && !seen.has(it.id)
        ))
        items = locals.concat(extras, next)
      } else {
      ...
      if (reset && !items.length) items = readCache()
      listReady = true
      renderGrid()
    } finally {
      loadingList = false
      if (pendingReload && gen === loadGen) {
        pendingReload = false
        loadPage(true)
      }
    }
  ...
  function openDetail(id) {
    ...
    try { window.closeNavMenusPublic?.() } catch (_) {}
    renderDetail()
    root.classList.remove('hidden')
    root.setAttribute('aria-hidden', 'false')
    document.body.classList.add('img-detail-open')
  ...
  function closeDetail() {
    ...
    document.body.classList.remove('img-detail-open')
    selectedId = null
  ...
  function renderDetail() {
    ...
    const bg = $('img-detail-bg')
    if (bg) bg.style.backgroundImage = src ? 'url(' + JSON.stringify(src) + ')' : ''
    const user = getUser?.() || {}
    const rawName = String(user.name || user.email || 'You').trim() || 'You'
    const display = rawName.includes('@') ? rawName.split('@')[0] : rawName
    const av = $('img-detail-avatar')
    const nm = $('img-detail-name')
    if (av) av.textContent = (display[0] || 'Y').toUpperCase()
  ...
  async function generateImage() {
    if (typeof window.requireToolAction === 'function' && !window.requireToolAction()) return
    ...
    await Promise.all(batch.map(async (slot, i) => {
      try {
        ...
        if ((e.status === 403 || e.needsUpgrade || (e.status === 402 && e.code === 'credits_exhausted')) && !window._imgPaywallOnce) {
  ...
  window.imgWorkspaceShow = () => {
    ...
    syncRefButton()
    syncGenerateEnabled()
    const token = window._getToken?.() || (typeof localStorage !== 'undefined' && localStorage.getItem('clipzo_token'))
    if (!token) {
      items = []
      listReady = true
      renderGrid()
  ...
  function onGridClick(e) {
    ...
    if (more) {
      ...
      const tile = more.closest('.img-tile')
      const open = !wrap.classList.contains('is-open')
      closePopovers()
      wrap.classList.toggle('is-open', open)
      tile?.classList.toggle('is-menu', open)
      return
    }
    const hit = e.target.closest('[data-open]')
    if (hit) {
      openDetail(hit.getAttribute('data-open'))
      return
    }
    const tile = e.target.closest('.img-tile')
    if (tile && !e.target.closest('.img-retry, [data-retry]')) {
      const openBtn = tile.querySelector('[data-open]')
  ...
  function bind() {
    ...
    $('img-detail-stage')?.addEventListener('click', (e) => {
    ...
    document.addEventListener('click', (e) => {
      if (!e.target.closest('.img-pop, .img-model, .img-more')) closePopovers()
    })
