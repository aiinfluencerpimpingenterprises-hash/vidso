...
/** Empty-state sample thumbnails. Fill with public R2 URLs; empty strings keep tinted cards. */
...
export function isMetaFileName(name) {
  const n = String(name || '')
  return n.startsWith(IMG_META_PREFIX) && n.endsWith(IMG_META_SUFFIX)
}

export function isHistorySidecarName(name) {
  const n = String(name || '')
  return isMetaFileName(n) || n.startsWith(IMG_REF_PREFIX)
}
