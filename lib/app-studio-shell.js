  ...
  scroller.innerHTML = list.map((f) => {
    ...
    return `<a class="studio-tile" href="${esc(url || '/files')}">
      <span class="studio-tile-shot">
        ...
        <span class="studio-tile-chip">${video ? 'Video' : 'Image'}</span>
      </span>
...
function startCards() {
  return START_FORMATS.map((f) => {
    ...
    const chip = f.chip ? `<span class="studio-tile-chip">${esc(f.chip)}</span>` : ''
    const ratio = f.ratio === '9:16' ? 'is-portrait' : 'is-wide'
    const goAttr = f.panel ? `data-go="${esc(f.panel)}" data-format="${esc(f.format || '')}"` : ''
    return `<button type="button" class="studio-tile" ${goAttr} aria-label="${esc(f.label)}">
      <span class="studio-tile-shot">
...
function newsCards() {
  return WHATS_NEW.map((n) => {
    ...
    const chip = n.chip ? `<span class="studio-tile-chip">${esc(n.chip)}</span>` : ''
    return `<button type="button" class="studio-tile" ${goAttr} aria-label="${esc(n.title || 'Announcement')}">
      <span class="studio-tile-shot">
...
function presetCards() {
  return HOME_PRESETS.map((t) => `
    <button type="button" class="studio-preset" data-go="${esc(t.panel)}" aria-label="${esc(t.title)}">
      <span class="studio-tile-shot">
        ${phHtml(appMedia(t.file), 'is-wide')}
        <span class="studio-scrim-grad"></span>
        <span class="studio-tile-chip">${t.panel === 'imagegen' ? 'Thumbnail' : 'Long-form'}</span>
      </span>
  ...
  const cells = [1, 2, 3, 4, 5, 6].map((n) => `
    <button type="button" class="studio-inspire-card is-n${n}" data-go="videogen" aria-label="${esc(cat.label)} ${n}">
      <span class="studio-tile-shot">
        ...
        <span class="studio-scrim-grad"></span>
        <span class="studio-tile-chip">${esc(cat.label)}</span>
      </span>
    </button>`).join('')
