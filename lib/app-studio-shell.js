...
function mountTopbarExtras() {
  ...
  if (help) {
    help.innerHTML = HELP_MENU.map((item) => {
      ...
      return `<a role="menuitem" href="${esc(href)}"${goAttr}${extra}>${menuIco(item.icon)}${esc(item.label)}</a>`
...
function paintAccountChrome() {
  ...
  if (av) {
    ...
    if (letter) {
      av.classList.remove('is-icon')
      av.textContent = letter.toUpperCase()
    } else {
      av.classList.add('is-icon')
      av.innerHTML = ICO.user
    }
  }
}
