  ...
  const parts = SIDEBAR_SECTIONS.map((sec) => {
    ...
    if (sec.pinned) {
      ...
      const body = items.length
        ...
        : `<p class="studio-nav-empty">Pin a tool from the tools list</p>`
      return `<div class="studio-nav-sec${closed ? ' is-shut' : ''}" data-sec="${esc(sec.id)}">${label}<div class="studio-nav-sec-body">${body}</div></div>`
    }
...
function navBtn(it, create) {
  ...
  const goAttr = it.external ? `data-href="${esc(it.href)}"` : `data-go="${esc(it.id)}"`
  const extra = create ? ' is-create' : ''
  return `<button type="button" class="studio-nav-item${extra}" data-panel="${esc(it.id)}" ${goAttr} title="${esc(it.name)}" aria-label="${esc(it.name)}">
...
function mountTopbarExtras() {
  ...
  if (left && !document.getElementById('studio-workspace-btn')) {
    ...
    wrap.innerHTML = `
      <button type="button" class="studio-workspace-btn" id="studio-workspace-btn" aria-haspopup="menu" aria-expanded="false" aria-controls="studio-workspace-menu">
        <span>${esc(WORKSPACE_PILL.label)}</span>
        <span aria-hidden="true">${ICO.chev}</span>
      </button>
  ...
  const grid = document.getElementById('studio-grid-btn')
  if (grid && !grid.innerHTML.trim()) grid.innerHTML = ICO.grid
  const help = document.getElementById('studio-help-menu')
  if (help) {
    help.classList.add('studio-drop-menu')
    const helpIco = { 'whats-new': 'templates', 'help-center': 'tutorials', feedback: 'commentary', email: 'captions', terms: 'files' }
    help.innerHTML = HELP_MENU.map((item) => {
      ...
      return `<a role="menuitem" href="${esc(href)}"${goAttr}${extra}>${menuIco(helpIco[item.id])}${esc(item.label)}</a>`
    }).join('') + `<div class="studio-help-socials" data-vidso-socials></div>`
  }
  const avatar = document.getElementById('nav-avatar-menu')
  if (avatar) {
    const info = avatar.querySelector('.user-info')
    ...
    AVATAR_MENU.forEach((item) => {
      const btn = document.createElement('button')
      btn.type = 'button'
      btn.setAttribute('role', 'menuitem')
      btn.dataset.avatar = item.action
      const icos = { profile: 'agents', paywall: 'ranking', settings: 'brandkit', logout: 'downloader' }
      btn.innerHTML = `${menuIco(icos[item.action] || 'agents')}${esc(item.label)}`
      avatar.appendChild(btn)
    })
    if (info) avatar.insertBefore(info, avatar.firstChild)
  }
  try { window.applyVidsoSocialLinks?.() } catch (_) {}
}
...
function modelCards() {
  return featuredModels().map((m) => {
    const slug = modelSlug(m.id)
    return `<a class="studio-tile studio-model-card" href="${esc('/models#' + slug)}" aria-label="${esc(m.name)}">
      <span class="studio-tile-shot">
        ${phHtml(modelCardFile(slug), 'is-wide')}
          ...
          <div class="studio-prompt-bar">
            ...
            <div class="studio-prompt-right">
              <div class="studio-pop" id="studio-ask-wrap">
                ...
                <div class="studio-mini-menu studio-drop-menu is-wide" id="studio-ask-menu" hidden role="menu">${askMenuHtml()}</div>
              </div>
...
function bindShellEvents() {
  document.addEventListener('click', (e) => {
    ...
    if (e.target.closest('#studio-grid-btn')) { openCmd(); return }
  ...
  document.addEventListener('keydown', (e) => {
    ...
    const menu = document.querySelector('.studio-mini-menu:not([hidden]), .studio-project-menu:not([hidden]), .studio-help-menu.is-open, #nav-avatar-menu')
