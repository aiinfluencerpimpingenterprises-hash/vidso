...
import { bindCatalogClipLoop, catalogClipWindow, catalogHasClip, featuredMediaId, featuredModels, modelById, modelCardClipSrc, modelCardSrc, modelSlug } from './vidso-models.js?v=s17'
import {
  ASK_BEHAVIORS,
  ATTACH_GROUPS,
  AVATAR_MENU,
...
function phHtml(src, extra = '', kind = 'img') {
  ...
  let media = ''
  if (url && kind === 'video') {
    media = `<video src="${esc(url)}" muted playsinline loop autoplay preload="metadata"></video>`
  } else if (url) {
...
function bindPh(root = document) {
  ...
  scope.querySelectorAll('[data-media-ph] video').forEach((vid) => {
    ...
    vid.addEventListener('error', () => {
      const card = vid.closest('.studio-inspire-card, .studio-inspire-mosaic .studio-preset')
      if (card) card.remove()
      else vid.remove()
    })
    const fit = () => {
      ...
      const ph = vid.closest('.studio-ph')
      if (!w || !h || !ph || ph.closest('.studio-tool-row, .studio-ref-thumb, .studio-pin-thumb')) return
  ...
  scope.querySelectorAll('.studio-model-card video').forEach((vid) => {
    ...
    bindCatalogClipLoop(vid, catalogClipWindow(id))
  })
}
...
function navBtn(it, create) {
  ...
  const ico = home
    ? `<span class="studio-nav-ico is-home-house" aria-hidden="true">${it.icon || ''}</span><span class="studio-nav-ico is-home-mark" aria-hidden="true"><img data-vidso-logo width="18" height="18" alt="" decoding="async"></span>`
...
function formatCard(f, { brief = true } = {}) {
  ...
  const src = startMedia(f.slug)
  const kind = /\.png$|\.jpe?g$|\.webp$/i.test(src) ? 'img' : 'video'
...
const HOME_RAIL_MODEL = { 'gpt-image-2': 'gpt-image-2-5' }

function modelCards() {
  return featuredModels().filter((m) => m.id !== 'claude').map((m) => {
    const id = HOME_RAIL_MODEL[m.id] || m.id
    const shown = modelById(id)
    const mediaId = featuredMediaId(id)
    const name = shown?.name || m.name
    ...
    return `<a class="studio-tile studio-model-card" data-model="${esc(mediaId)}" href="${esc('/models#' + modelSlug(mediaId))}" aria-label="${esc(name)}">
      <span class="studio-tile-shot">
        ${media}
        ${shown?.isNew || m.isNew ? '<span class="studio-badge">New</span>' : ''}
      </span>
      <span class="studio-tile-meta">${esc(name)}</span>
    </a>`
  }).join('')
}
...
function paintInspire(catId) {
  ...
  const cells = items.slice(0, 6).map((it, i) => `
    <button type="button" class="studio-inspire-card is-n${i + 1}" data-inspire-open="${esc(it.id)}" aria-label="${esc(it.title)}">
      <span class="studio-tile-shot">
        ${phHtml(it.src || appMedia(it.file), '', it.src ? 'video' : 'img')}
        <span class="studio-scrim-grad"></span>
...
function mountInspireCat() {
  ...
  root.innerHTML = `<div class="studio-page">
    ...
    <div class="studio-inspire-mosaic">${items.map((it, i) => `
...
function paintBrief() {
  ...
  if (preview) {
    const src = startMedia(f.slug)
    const kind = /\.png$|\.jpe?g$|\.webp$/i.test(src) ? 'img' : 'video'
    preview.innerHTML = phHtml(src, f.ratio === '9:16' ? 'is-portrait' : 'is-wide', src ? kind : 'img')
    bindPh(preview)
  }
...
function bindShellEvents() {
  document.addEventListener('click', (e) => {
    ...
    const homeBtn = e.target.closest('.studio-nav-item.is-home')
    if (homeBtn && document.body.classList.contains('studio-collapsed')) {
      e.preventDefault()
