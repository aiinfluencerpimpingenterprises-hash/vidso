...
export function mcpConnectorUrl(mcpUrl, token) {
  const base = String(mcpUrl || (apiOrigin() + '/api/mcp')).split('?')[0]
  if (!token) return base
  return base + (base.includes('?') ? '&' : '?') + 'token=' + encodeURIComponent(token)
}

export function mcpConfigJson(mcpUrl, token) {
  const url = mcpConnectorUrl(mcpUrl, token)
  const server = { url }
  if (token) server.headers = { Authorization: 'Bearer ' + token }
  return JSON.stringify({ mcpServers: { vidso: server } }, null, 2)
}

export async function youtubeUploads() {
  return ytReq('GET', '/api/youtube/uploads')
}

export async function youtubeRecordUpload(row) {
  return ytReq('POST', '/api/youtube/record', row)
}

export async function mcpConnectionStatus() {
  return ytReq('GET', '/api/mcp/status')
}

export async function mcpIssueToken() {
  return ytReq('POST', '/api/mcp/token', {})
}

export async function mcpRevokeToken() {
  return ytReq('POST', '/api/mcp/revoke', {})
}
...
export async function uploadViaServer({ renderJobId, videoUrl, title, description, privacy, tags, project }) {
