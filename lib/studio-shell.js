...
function providerMark(group) {
  const g = String(group || '').toLowerCase()
  if (g === 'google') {
    return `<svg class="fs-model-card-mark" viewBox="0 0 24 24" aria-hidden="true"><path fill="#4285F4" d="M23.5 12.3c0-.8-.1-1.6-.2-2.3H12v4.4h6.4c-.3 1.5-1.1 2.8-2.4 3.6v3h3.9c2.3-2.1 3.6-5.2 3.6-8.7z"/><path fill="#34A853" d="M12 24c3.2 0 5.9-1.1 7.9-2.9l-3.9-3c-1.1.7-2.5 1.2-4 1.2-3.1 0-5.7-2.1-6.6-4.9H1.4v3.1C3.4 21.4 7.4 24 12 24z"/><path fill="#FBBC05" d="M5.4 14.4c-.2-.7-.4-1.5-.4-2.4s.1-1.7.4-2.4V6.5H1.4C.5 8.4 0 10.2 0 12s.5 3.6 1.4 5.5l4-3.1z"/><path fill="#EA4335" d="M12 4.8c1.8 0 3.3.6 4.6 1.8l3.4-3.4C17.9 1.1 15.2 0 12 0 7.4 0 3.4 2.6 1.4 6.5l4 3.1C6.3 6.8 8.9 4.8 12 4.8z"/></svg>`
  }
  const letter = esc((group || '?').slice(0, 1).toUpperCase())
  return `<span class="fs-model-card-letter">${letter}</span>`
}

function modelCardHtml(m, picked) {
  const on = m.id === picked
  return `<button type="button" class="fs-model-card${on ? ' is-on' : ''}" role="menuitem" data-video-model="${esc(m.id)}"${on ? ' aria-checked="true"' : ''}>
    <span class="fs-model-card-top">
      ${providerMark(m.group)}
      <span class="fs-model-card-badge">${esc(m.badge || 'Video')}</span>
    </span>
    <span class="fs-model-card-copy">
      <strong>${esc(m.name)}</strong>
      <small>${esc(m.blurb || m.hint || '')}</small>
    </span>
  </button>`
}

export function videoModelMenuHtml(picked) {
  const stock = { id: STOCK_VIDEO_MODEL, name: 'Stock footage', group: 'Pexels', blurb: 'Pexels B-roll', badge: 'Video' }
  return `<div class="fs-menu fs-model-menu" id="fs-model-menu" role="menu">
    ...
    ${modelCardHtml(stock, picked)}
    ${VIDEO_MODELS.map((m) => modelCardHtml(m, picked)).join('')}
  </div>`
}
...
export function connectorsMenuHtml() {
  return `<div class="fs-menu fs-conn-menu" id="fs-conn-menu" role="menu">
    ...
    <button type="button" role="menuitem" data-conn="youtube" data-yt-mcp="1" data-archived hidden>
      <img src="${esc(YOUTUBE_LOGO_URL)}" alt="" width="18" height="18"> YouTube MCP
    </button>
    <a role="menuitem" class="fs-menu-foot" href="/youtube?yt=1" data-archived hidden>Manage connectors</a>
  </div>`
}
...
export function composerHtml(opts) {
  ...
  const creditTitle = creditCost
    ? (creditsLeft == null
      ? creditCost + ' credits for this generate'
      : creditCost + ' credits · ' + creditsLeft + ' left this month')
    : ''
  return `<form class="fs-composer fs-composer-sc" id="fs-composer" autocomplete="off">
    ...
    <div class="fs-composer-bar">
      ...
      <div class="fs-composer-right">
        ${creditCost ? `<span class="fs-credit-cost" id="fs-credit-cost" title="${esc(creditTitle)}">${esc(String(creditCost))} cr</span>` : '<span class="fs-credit-cost" id="fs-credit-cost" hidden></span>'}
    ...
    ${temp ? '' : `<div class="fs-tool-row">
      ...
      <button type="button" class="fs-tool" id="fs-connectors-btn" aria-haspopup="menu" data-archived hidden>${icon('plug')} Connectors ${connectorLogosHtml()}</button>
      <button type="button" class="fs-tool fs-tool-mcp" id="fs-yt-mcp" data-archived hidden>YouTube MCP</button>
    </div>`}
...
export function homeHeroHtml(temp) {
  ...
  return `<div class="fs-hero fs-hero-center">
    ...
    <h1 class="fs-hero-title">Start a faceless video</h1>
...
function heroFanVideos() {
  return Array.from(document.querySelectorAll('#fs-hero-fan video'))
}

export function playHeroFan(root) {
  const scope = root || document
  const vids = Array.from(scope.querySelectorAll('#fs-hero-fan video'))
  ...
  for (const v of vids) {
    ...
    if (document.visibilityState !== 'hidden') v.play?.().catch(() => {})
  }
  // Four looping clips keep decoding on a background tab, which is what made
  // the studio feel sluggish after it had been left open.
  if (heroVisibilityBound) return
  heroVisibilityBound = true
  document.addEventListener('visibilitychange', () => {
    const hidden = document.visibilityState === 'hidden'
    for (const v of heroFanVideos()) {
      if (hidden) { try { v.pause() } catch (_) {} }
      else v.play?.().catch(() => {})
    }
  })
}
...
export function connectorsBoardHtml() {
  return `<div class="fs-board">
    ...
    <div class="fs-conn-grid">
      <button type="button" class="fs-conn-card" id="fs-yt-mcp-card" data-yt-mcp="1" data-archived hidden>
        <img src="${esc(YOUTUBE_LOGO_URL)}" alt="" width="36" height="36">
        <strong>YouTube MCP</strong>
        <span>Connect a channel in Vidso. Finished videos can upload without leaving the app.</span>
      </button>
    </div>
    <p><a class="btn btn-ghost" href="/youtube?yt=1" data-archived hidden>Manage connectors</a></p>
  </div>`
}
...
export function showMenu(html, anchor) {
  ...
  const wide = menu.classList.contains('fs-model-menu')
  const mw = wide ? Math.min(680, window.innerWidth - 24) : 320
