...
export function durationFromBody(body) {
  ...
  const direct = Number(body.duration || body.durationSeconds || body.duration_seconds)
  if (Number.isFinite(direct) && direct > 0) return direct
  const id = String(body.duration_id || body.durationId || '')
...
function accessFromUser(user) {
  ...
  const fromId = resolveWhopPlan(user.whop_plan_id || user.plan_id)
  const tier = fromId.status === 'mapped' ? fromId.tier : normalizeTier(user.plan || user.plan_tier || user.tier)
  const legacy = fromId.legacy === true && !tier
  return { active, tier, legacy, entitlements: entitlementsFor(tier) }
}
