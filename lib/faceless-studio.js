...
import { DEFAULT_VIDEO_MODEL, STOCK_VIDEO_MODEL, VIDEO_MODELS } from '/lib/fal-video.js'
...
export function initFacelessStudio(deps) {
  ...
  function bindComposer() {
    ...
    $('fs-video-model')?.addEventListener('change', (e) => {
      ...
      try { window.fvSetStudioVideoModel?.(e.target.value) } catch (_) {}
  ...
  async function onGenerate(e) {
    ...
    try {
      ...
      await studioReq('POST', '/projects/' + project.id + '/jobs', {
        ...
        progress: 100,
        provider: 'vidso',
        parameters: { length: picked.id, duration_seconds: picked.seconds, aspect: compose.aspect, voice_id: compose.voice, video_model: compose.videoModel || DEFAULT_VIDEO_MODEL },
      })
      if (!isTempChat()) {
