...
export function initFacelessStudio(deps) {
  ...
  function bindComposer() {
    ...
    $('fs-video-model')?.addEventListener('click', (e) => openVideoModelMenu(e.currentTarget))
    $('fs-aspect')?.addEventListener('click', (e) => openAspectMenu(e.currentTarget))
    $('fs-clip-duration')?.addEventListener('click', (e) => openDurationMenu(e.currentTarget))
    $('fs-resolution')?.addEventListener('click', (e) => openResolutionMenu(e.currentTarget))
    $('fs-sound')?.addEventListener('click', () => {
      const model = videoModelById(compose.videoModel)
      if (!isFalVideoModel(compose.videoModel) || !model?.audio) return
      compose.sound = !compose.sound
      persistVideoSettings()
      if (current?.id) scheduleSave()
      if (view !== 'project') renderMain()
    })
    $('fs-count-minus')?.addEventListener('click', () => {
      compose.generations = clampVideoCount(compose.generations - 1)
      persistVideoSettings()
      if (current?.id) scheduleSave()
      const label = $('fs-count-label')
      if (label) label.textContent = compose.generations + '/4'
    })
    $('fs-count-plus')?.addEventListener('click', () => {
      compose.generations = clampVideoCount(compose.generations + 1)
      persistVideoSettings()
      if (current?.id) scheduleSave()
      const label = $('fs-count-label')
  ...
  function applyFalBarState() {
    const fal = isFalVideoModel(compose.videoModel)
    const model = videoModelById(compose.videoModel)
    const audioOk = fal && !!model?.audio
    ;['fs-clip-duration', 'fs-resolution', 'fs-count-minus', 'fs-count-plus'].forEach((id) => {
      const el = $(id)
      if (el) el.disabled = !fal
    })
    const sound = $('fs-sound')
    if (sound) sound.disabled = !audioOk
    $('fs-clip-duration')?.classList.toggle('is-off', !fal)
    $('fs-resolution')?.classList.toggle('is-off', !fal)
    $('fs-count')?.classList.toggle('is-off', !fal)
    sound?.classList.toggle('is-off', !audioOk)
  }

  function commitVideoSetting() {
    persistVideoSettings()
    if (current?.id) scheduleSave()
    paintVideoBar()
  }

  function openVideoModelMenu(anchor) {
    const menu = showMenu(videoModelMenuHtml(compose.videoModel), anchor)
    if (!menu) return
    ...
    menu.querySelectorAll('[data-video-model]').forEach((btn) => {
      btn.addEventListener('click', () => {
        compose.videoModel = btn.getAttribute('data-video-model') || STOCK_VIDEO_MODEL
        const model = videoModelById(compose.videoModel)
        if (model) {
          compose.clipDuration = clipDurationFor(compose.videoModel, compose.clipDuration)
          compose.resolution = resolutionFor(compose.videoModel, compose.resolution)
          if (!model.audio) compose.sound = false
        } else {
          compose.resolution = ''
        }
        closeStudioMenus()
  ...
  function openAspectMenu(anchor) {
    const opts = isFalVideoModel(compose.videoModel) ? aspectsForModel(compose.videoModel) : ['16:9', '9:16']
    const menu = showMenu(aspectMenuHtml(opts, compose.aspect), anchor)
    menu?.querySelectorAll('[data-aspect]').forEach((btn) => {
      btn.addEventListener('click', () => {
        compose.aspect = btn.getAttribute('data-aspect') || '16:9'
        closeStudioMenus()
        commitVideoSetting()
      })
    })
  ...
  function openDurationMenu(anchor) {
    if (!isFalVideoModel(compose.videoModel)) return
    const list = durationsForModel(compose.videoModel)
    const menu = showMenu(durationMenuHtml(list, compose.clipDuration), anchor)
    if (!menu) return
    const apply = (raw) => {
      const n = Number(String(raw || '').replace(/s$/i, ''))
      compose.clipDuration = clipDurationFor(compose.videoModel, n)
      closeStudioMenus()
      commitVideoSetting()
    }
    menu.querySelectorAll('[data-duration]').forEach((btn) => {
      btn.addEventListener('click', () => apply(btn.getAttribute('data-duration')))
    })
  ...
  function openResolutionMenu(anchor) {
    const list = resolutionsForModel(compose.videoModel)
    if (!list.length) return
    const menu = showMenu(resolutionMenuHtml(list, compose.resolution), anchor)
    menu?.querySelectorAll('[data-resolution]').forEach((btn) => {
      btn.addEventListener('click', () => {
        compose.resolution = resolutionFor(compose.videoModel, btn.getAttribute('data-resolution'))
        closeStudioMenus()
        commitVideoSetting()
      })
    })
  ...
  document.addEventListener('click', (e) => {
    if (e.target.closest('#fs-pop .fs-menu, #fs-ref-add, #fs-ref-images-btn, #fs-ask-btn, #fs-project-btn, #fs-connectors-btn, #fs-skills-btn, #fs-mem-plus, #fs-auto-btn, #fs-video-model, #fs-aspect, #fs-clip-duration, #fs-resolution, #fs-sound, #fs-count, [data-fs-view="search"]')) return
