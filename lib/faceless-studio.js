...
export function initFacelessStudio(deps) {
  ...
  async function queueFalVideo() {
    ...
    const count = clampVideoCount(current.video_count || current.pipeline?.generations || compose.generations)
    ...
    for (let i = 0; i < count; i++) {
      setFalStatus('Generating ' + videoModelLabel(modelId) + (count > 1 ? ' ' + (i + 1) + ' of ' + count : '') + '…')
      try {
        ...
        const added = await studioReq('POST', '/projects/' + current.id + '/assets', {
          ...
          storage_url: url,
          mime: 'video/mp4',
          label: count > 1 ? 'Clip ' + (i + 1) : videoModelLabel(modelId),
        })
    ...
    if (!urls.length && lastErr) falRun.error = lastErr
    renderMain()
    markRail()
  }
