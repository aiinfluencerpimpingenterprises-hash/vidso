...
export function initFacelessStudio(deps) {
  ...
  function composerHtml() {
    ...
    const voiceOk = voiceState === 'ready' && voices().length > 0
    const genDisabled = composerBusy || !voiceOk
    const genTitle = voiceState === 'loading' || voiceState === 'idle'
    ...
    return `<form class="fs-composer" id="fs-composer" autocomplete="off">
      ...
      <div class="fs-composer-row">
        ...
        <button type="submit" class="btn btn-primary fs-gen" id="fs-gen"${genDisabled ? ' disabled' : ''}${genTitle ? ` title="${esc(genTitle)}"` : ''}>${composerBusy ? 'Creating…' : 'Generate'}</button>
      </div>
