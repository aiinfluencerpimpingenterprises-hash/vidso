...
const HOW_CARDS = [
  { title: 'Pick a topic', caption: 'Describe the video you want and choose length, aspect, and voice.', src: LFG_STEP_SHOTS.script },
  { title: 'Review the cut', caption: 'Edit the script, swap B-roll, style captions, and set music.', src: LFG_STEP_SHOTS.media },
  { title: 'Export', caption: 'Render the final MP4 and generate a matching thumbnail.', src: LFG_STEP_SHOTS.export },
]
...
export function initFacelessStudio(deps) {
      ...
      ${sections.map((s) => `<section class="fs-g-sec" aria-labelledby="fs-g-${esc(s.id)}">
        ...
        <div class="fs-g-grid">
          ${s.cards.map((c) => `<button type="button" class="fs-preset" data-preset="${esc(c.id)}">
            ${mediaPlaceholderHtml({ src: c.image, ratio: '16 / 9', className: 'fs-preset-shot' })}
  ...
  function queueAutoScript() {
    const run = async () => {
      ...
      if (compose.voice) window.pickFvVoice?.(compose.voice)
      try {
        await studioReq('POST', '/projects/' + current.id + '/jobs', { type: 'script', status: 'running', progress: 5 })
      } catch (_) {}
      try {
