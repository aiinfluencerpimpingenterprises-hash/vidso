...
import { bindShowcaseGrid, templatesBoardHtml } from '/lib/studio-showcase.js'
import {
  addMemory,
  getActiveProjectId,
  getAskMode,
...
export function initFacelessStudio(deps) {
  ...
  function feedTileHtml(it) {
    ...
    if (it.pending) {
      return `<article class="fs-shot fs-feed-tile is-pending" style="aspect-ratio:${ar}" aria-busy="true">
        <div class="img-tile-skel"></div>
        <span class="fs-feed-wait">${esc(falRun.label || 'Generating…')}</span>
      </article>`
    }
    if (it.error && !it.url) {
      return `<article class="fs-shot fs-feed-tile is-error" style="aspect-ratio:${ar}">
    ...
    return `<article class="fs-shot fs-feed-tile${it.aspect === '9:16' ? ' is-9x16' : ''}" style="aspect-ratio:${ar}" data-feed-url="${esc(it.url)}">
      <video class="fs-shot-vid" muted loop playsinline preload="metadata" src="${esc(it.url)}" aria-hidden="true"></video>
  ...
  function paintFeed() {
    ...
    grid.innerHTML = feed.map(feedTileHtml).join('')
    bindFeedGrid($('fs-main'))
  }
  ...
  function bindFeedGrid(root) {
    if (!root) return
    root.querySelectorAll('.fs-feed-tile').forEach((card) => {
      const vid = card.querySelector('video')
      card.addEventListener('mouseenter', () => { try { vid?.play() } catch (_) {} })
      card.addEventListener('mouseleave', () => { try { vid?.pause() } catch (_) {} })
      card.addEventListener('click', (e) => {
        if (e.target.closest('[data-feed-retry]')) return
        const url = card.getAttribute('data-feed-url')
        if (url) openFeedViewer(url)
      })
    })
    root.querySelectorAll('[data-feed-retry]').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.preventDefault()
        e.stopPropagation()
        const slot = feed.find((it) => it.localId === btn.getAttribute('data-feed-retry'))
        if (slot?.projectId) {
  ...
  function projectCard(p) {
    ...
    const media = clip
      ? `<video class="fs-proj-thumb" src="${esc(clip)}" muted playsinline preload="metadata"></video>`
  ...
  async function go(next) {
    ...
    if (next === 'memory' || next === 'connectors' || next === 'templates') renderMain()
    else refreshProjects()
  }
