...
export function initFacelessStudio(deps) {
  ...
  function videoModelInnerHtml() {
    const picked = compose.videoModel || DEFAULT_VIDEO_MODEL
    const stock = `<option value="${STOCK_VIDEO_MODEL}"${picked === STOCK_VIDEO_MODEL ? ' selected' : ''}>Stock footage</option>`
    const groups = []
    VIDEO_MODELS.forEach((m) => {
      ...
      g.rows.push(`<option value="${esc(m.id)}"${m.id === picked ? ' selected' : ''}>${esc(m.name)}</option>`)
    })
    return stock + groups.map((g) => `<optgroup label="${esc(g.label)}">${g.rows.join('')}</optgroup>`).join('')
  }

  function aspectInnerHtml() {
    const opts = isFalVideoModel(compose.videoModel) ? aspectsForModel(compose.videoModel) : ['16:9', '9:16']
    const picked = opts.includes(compose.aspect) ? compose.aspect : opts[0]
    compose.aspect = picked
    return opts.map((a) => `<option value="${esc(a)}"${a === picked ? ' selected' : ''}>${esc(a)}</option>`).join('')
  }

  function clipDurationInnerHtml() {
    const list = isFalVideoModel(compose.videoModel) ? durationsForModel(compose.videoModel) : [5]
    const picked = clipDurationFor(compose.videoModel, compose.clipDuration)
    compose.clipDuration = picked
    return list.map((n) => `<option value="${n}"${n === picked ? ' selected' : ''}>${n}s</option>`).join('')
  }
  ...
  function composerHtml() {
    ...
    return shellComposerHtml({
      ...
      aspectInner: aspectInnerHtml(),
      clipDurationInner: clipDurationInnerHtml(),
      videoModelInner: videoModelInnerHtml(),
  ...
  function bindComposer() {
    ...
    $('fs-aspect')?.addEventListener('change', (e) => {
      compose.aspect = e.target.value
      persistVideoSettings()
      if (current?.id) scheduleSave()
    })
    $('fs-video-model')?.addEventListener('change', (e) => {
      compose.videoModel = e.target.value
    ...
    $('fs-clip-duration')?.addEventListener('change', (e) => {
      compose.clipDuration = Number(e.target.value) || compose.clipDuration
      persistVideoSettings()
      if (current?.id) scheduleSave()
    })
  ...
  function applyFalBarState() {
    ...
    ;['fs-clip-duration', 'fs-count-minus', 'fs-count-plus'].forEach((id) => {
      const el = $(id)
      if (el) el.disabled = !fal
    })
    const sound = $('fs-sound')
    if (sound) sound.disabled = !audioOk
    $('fs-clip-duration-chip')?.classList.toggle('is-off', !fal)
  ...
  document.addEventListener('click', (e) => {
    if (e.target.closest('#fs-pop .fs-menu, #fs-ref-add, #fs-ask-btn, #fs-project-btn, #fs-connectors-btn, #fs-skills-btn, #fs-mem-plus, #fs-auto-btn, [data-fs-view="search"]')) return
