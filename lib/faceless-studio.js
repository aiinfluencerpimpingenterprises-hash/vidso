 ...
 * Home generate sends the prompt to Fal (same path as the thumbnail
 * generator). The four-stage #fv-wizard stays available for stock / long-form.
 */
...
import { creditCharge, studioCreditView } from '/lib/studio-credits.js'
import { studioGateHref, studioGateRest } from '/lib/studio-gate.js'
...
import { bindShowcaseGrid, lazyVideos, templatesBoardHtml } from '/lib/studio-showcase.js'
import {
  addMemory,
  getActiveProjectId,
  getAskMode,
...
import { generationCardHtml, generationProgressToward } from '/lib/studio-gen-card.js'
import {
  closeStudioMenus,
...
export function initFacelessStudio(deps) {
  ...
  let pendingFalVideo = false
  let falRun = { active: false, label: '', error: '' }
  let genCard = { shown: false, startedAt: 0, progress: 0, error: '', done: false }
  let genTick = null
  let feed = []
  let saveTimer = null
  ...
  let saveError = ''
  let studioCredits = { remaining: null, used: 0, limit: 0 }
  ...
  function pathForView() {
    ...
    if (view === 'templates') return '/faceless-studio/templates'
  ...
  function parsePath() {
    ...
    if (p === '/faceless-studio/templates') return { view: 'templates' }
  ...
  async function studioReq(method, path, body, opts = {}) {
    ...
    const rest = studioGateRest(path)
    if (rest) headers['x-vidso-studio'] = rest
    ...
    if (!res.ok) {
      ...
      const err = new Error(data.error || data.message || (typeof data.raw === 'string' ? data.raw.slice(0, 180) : '') || fallback)
  ...
  function currentClipCredits() {
    if (!isFalVideoModel(compose.videoModel)) return 0
    return creditCharge({
      kind: 'video',
      model: compose.videoModel,
      duration: compose.clipDuration,
      resolution: compose.resolution,
      generateAudio: !!compose.sound,
    })
  }

  function currentGenerateCredits() {
    return currentClipCredits() * clampVideoCount(compose.generations)
  }

  function paintCreditChip() {
    const el = $('fs-credit-cost')
    if (!el) return
    const fal = isFalVideoModel(compose.videoModel)
    const cost = fal ? currentGenerateCredits() : 0
    el.hidden = !fal || !cost
    if (!cost) return
    el.textContent = cost + ' cr'
    const left = studioCredits.remaining
    el.title = left == null
      ? cost + ' credits for this generate'
      : cost + ' credits · ' + left + ' left this month'
  }

  function applyCreditsFromUser(user) {
    const u = user || getUser?.()
    if (!u) return
    studioCredits = studioCreditView(u, u)
    paintCreditChip()
  }

  function applyCreditsFromGenerate(data) {
    if (!data || data.creditsRemaining == null) return
    const limit = Number(data.creditsLimit) || studioCredits.limit || 0
    studioCredits = {
      remaining: Math.max(0, Number(data.creditsRemaining) || 0),
      used: Math.max(0, limit - (Number(data.creditsRemaining) || 0)),
      limit,
    }
    paintCreditChip()
  }
  ...
  function composerHtml() {
    ...
    const genDisabled = composerBusy || falRun.active || (signedIn && !fal && !voiceOk)
    const genTitle = !signedIn
      ? 'Sign in to generate'
      : (fal ? '' : (voiceState === 'loading' || voiceState === 'idle'
        ? 'Voices are still loading'
        : (!voiceOk ? 'Voices are not available. Retry, then generate.' : '')))
    const temp = view === 'temp' || isTempChat()
    syncVideoDefaults()
    const creditCost = fal ? currentGenerateCredits() : 0
    return shellComposerHtml({
      ...
      temp,
      creditCost,
      creditsLeft: studioCredits.remaining,
    })
  }
  ...
  function galleryHtml() {
    if (feed.length || falRun.active) return feedGridHtml()
    return ''
  }

  function isFalDirectProject(p) {
    if (falRun.active) return true
    if (p?.pipeline?.falDirect) return true
    const model = p?.video_model || p?.pipeline?.videoModel
    if (!isFalVideoModel(model) || p?.pipeline?.script) return false
    return true
  }

  function brollClips(p) {
    return (p?.assets || []).filter((a) => a.type === 'broll' && a.storage_url)
  }

  function shouldAutostartFal(p) {
    if (!p?.id || falRun.active || !isFalDirectProject(p)) return false
    if (brollClips(p).length) return false
    if (!String(p.topic || '').trim()) return false
    return !(p.jobs || []).some((j) => j.type === 'video')
  }
  ...
  function hydrateFeed() {
    const pending = feed.filter((it) => it.pending)
    const errors = feed.filter((it) => it.error && !it.pending)
    const seen = new Set(feed.map((it) => it.url).filter(Boolean))
    const remote = []
    for (const p of projects) {
      for (const a of brollClips(p)) {
        if (!a.storage_url || seen.has(a.storage_url)) continue
        seen.add(a.storage_url)
        remote.push({
          id: a.id || a.storage_url,
          url: a.storage_url,
          prompt: p.topic || p.title || '',
    ...
    feed = pending.concat(remote, errors)
  }

  /** Identity of a tile's rendered shape, so repaints can skip untouched tiles. */
  function feedTileKey(it) {
    const state = it.pending ? 'pending' : (it.url ? 'ready' : 'error')
    return [it.localId || it.id || it.url || '', state, it.url || '', it.error || ''].join('|')
  }

  function feedTileHtml(it) {
    const ar = it.aspect === '9:16' ? '9 / 16' : '16 / 9'
    const key = esc(feedTileKey(it))
    if (it.pending) {
      return `<article class="fs-shot fs-feed-tile is-pending" style="aspect-ratio:${ar}" data-feed-key="${key}" aria-busy="true">
        <div class="img-tile-skel"></div>
        <span class="fs-feed-wait">${esc(falRun.label || 'Generating…')}</span>
      </article>`
    }
    if (it.error && !it.url) {
      return `<article class="fs-shot fs-feed-tile is-error" style="aspect-ratio:${ar}" data-feed-key="${key}">
        <div class="fs-feed-fail">
          <p>${esc(it.error)}</p>
          <button type="button" class="fs-recreate" data-feed-retry="${esc(it.localId || '')}">Retry</button>
        </div>
      </article>`
    }
    return `<article class="fs-shot fs-feed-tile${it.aspect === '9:16' ? ' is-9x16' : ''}" style="aspect-ratio:${ar}" data-feed-key="${key}" data-feed-url="${esc(it.url)}">
      <video class="fs-shot-vid" muted loop playsinline preload="metadata" data-src="${esc(it.url)}" aria-hidden="true"></video>
      <div class="fs-shot-hover">
        <p class="fs-shot-prompt">${esc(it.prompt || '')}</p>
      </div>
    </article>`
  }

  function feedGridHtml() {
    const portrait = feed.some((it) => it.aspect === '9:16')
    return `<div class="fs-gallery" id="fs-feed">
      <div class="fs-g-grid fs-shot-grid fs-feed-grid${portrait ? ' is-portrait' : ''}" id="fs-feed-grid">
        ${feed.map(feedTileHtml).join('')}
      </div>
    </div>`
  }

  /**
   * Patch the grid in place. Replacing innerHTML on every progress tick threw
   * away every <video>, so finished clips reloaded once per status update.
   */
  function paintFeed() {
    const grid = $('fs-feed-grid')
    if (!grid) {
      if (view === 'home' || view === 'temp') renderMain()
      return
    }
    const portrait = feed.some((it) => it.aspect === '9:16')
    grid.classList.toggle('is-portrait', portrait)

    const alive = new Map()
    grid.querySelectorAll('.fs-feed-tile').forEach((el) => {
      const key = el.getAttribute('data-feed-key')
      if (key && !alive.has(key)) alive.set(key, el)
      else el.remove()
    })

    let cursor = null
    for (const it of feed) {
      const key = feedTileKey(it)
      let el = alive.get(key)
      if (el) {
        alive.delete(key)
        if (it.pending) {
          const wait = el.querySelector('.fs-feed-wait')
          const label = falRun.label || 'Generating…'
          if (wait && wait.textContent !== label) wait.textContent = label
        }
      } else {
        const holder = document.createElement('div')
        holder.innerHTML = feedTileHtml(it)
  ...
  function ensureFeedViewer() {
    let el = $('fs-feed-view')
    if (el) return el
    el = document.createElement('div')
    el.id = 'fs-feed-view'
    el.className = 'fs-feed-view'
    el.hidden = true
    el.innerHTML = `<div class="fs-feed-view-scrim" data-feed-close></div>
      <div class="fs-feed-view-frame">
        <button type="button" class="fs-feed-view-x" data-feed-close aria-label="Close">×</button>
        <video id="fs-feed-view-video" controls playsinline></video>
      </div>`
    el.addEventListener('click', (e) => {
      if (e.target.closest('[data-feed-close]')) closeFeedViewer()
    })
    document.addEventListener('keydown', (e) => {
      if (e.key !== 'Escape') return
      if (!$('fs-feed-view') || $('fs-feed-view').hidden) return
      closeFeedViewer()
    })
  ...
  function closeFeedViewer() {
    const el = $('fs-feed-view')
    const video = $('fs-feed-view-video')
    if (video) {
    ...
    document.body.classList.remove('fs-feed-view-open')
  }

  function openFeedViewer(url) {
    if (!url) return
    const el = ensureFeedViewer()
    const video = $('fs-feed-view-video')
    if (video) {
      video.src = url
      video.play?.().catch(() => {})
    ...
    el.hidden = false
    document.body.classList.add('fs-feed-view-open')
  }

  // Delegated once per grid: tiles are added and removed as clips land, and
  // per-tile listeners had to be re-attached on every repaint.
  function bindFeedGrid(root) {
    const grid = (root || document).querySelector('#fs-feed-grid')
    if (!grid) return
    lazyVideos(grid)
    if (grid.dataset.fsBound === '1') return
    grid.dataset.fsBound = '1'

    grid.addEventListener('pointerover', (e) => {
      const card = e.target.closest?.('.fs-feed-tile')
    ...
    grid.addEventListener('pointerout', (e) => {
      const card = e.target.closest?.('.fs-feed-tile')
      if (!card || card.contains(e.relatedTarget)) return
      try { card.querySelector('video')?.pause() } catch (_) {}
    })
    grid.addEventListener('click', (e) => {
      const retry = e.target.closest('[data-feed-retry]')
      if (retry) {
        e.preventDefault()
        e.stopPropagation()
        const slot = feed.find((it) => it.localId === retry.getAttribute('data-feed-retry'))
        if (slot?.projectId) {
          const p = projects.find((x) => x.id === slot.projectId) || current
          if (p) {
            current = p
            queueFalVideo([slot])
          }
      ...
      const card = e.target.closest('.fs-feed-tile')
      const url = card?.getAttribute('data-feed-url')
      if (url) openFeedViewer(url)
    })
  }

  function projectCard(p) {
    const clip = brollClips(p)[0]?.storage_url || ''
    const thumb = (p.assets || []).find((a) => a.type === 'thumbnail')?.storage_url || ''
    const media = clip
      ? `<video class="fs-proj-thumb" data-src="${esc(clip)}" muted playsinline preload="metadata"></video>`
  ...
  function renderMain() {
    ...
    if (view === 'project' && current) {
      ...
      if (!isFalDirectProject(current) && $('fs-pipeline-slot')) {
      ...
      if (pendingFalVideo) {
        pendingFalVideo = false
        queueFalVideo()
      } else if (shouldAutostartFal(current)) {
        queueFalVideo()
      } else if (pendingAutoScript) {
    ...
    if (view === 'projects' || view === 'favorites') {
      ...
      const temp = view === 'temp' || isTempChat()
      const hasFeed = !!(feed.length || falRun.active)
      main.innerHTML = `<div class="fs-home has-composer is-docked${temp ? ' is-temp' : ''}${hasFeed ? ' has-feed' : ''}">
  ...
  function falStageHtml(p) {
    const clips = brollClips(p)
    const model = videoModelLabel(p.video_model || compose.videoModel)
    const ratio = (p.aspect === '9:16' ? '9 / 16' : '16 / 9')
    const status = falRun.active
      ? (falRun.label || ('Generating ' + model + '…'))
      : (falRun.error && !clips.length ? falRun.error : '')
    const statusClass = falRun.error && !falRun.active && !clips.length ? ' is-err' : ''
    return `<section class="fs-fal-stage">
      <h2>${esc(model)}</h2>
      <p class="fs-fal-status${statusClass}" id="fs-fal-status"${status ? '' : ' hidden'}>${esc(status || (falRun.active ? 'Generating…' : ''))}</p>
      <div class="fs-fal-grid" id="fs-fal-grid">
        ${clips.map((a) => `<div class="fs-fal-card"><video src="${esc(a.storage_url)}" controls playsinline preload="metadata" style="aspect-ratio:${ratio}"></video></div>`).join('')}
        ${falRun.active && !clips.length ? '<div class="fs-fal-card is-wait" aria-busy="true"><span class="spinner"></span><span>Generating the clip…</span></div>' : ''}
      </div>
      <div class="fs-fal-actions">
        <button type="button" class="btn btn-primary" id="fs-fal-again"${falRun.active ? ' disabled' : ''}>${clips.length ? 'Generate again' : 'Generate video'}</button>
      </div>
    </section>`
  }
  ...
  function workspaceHtml() {
    const p = current
    const falDirect = isFalDirectProject(p)
    ...
    return `<div class="fs-workspace">
      <div class="fs-ws-head">
        ...
        <span class="fs-ws-status">${esc(falRun.active ? 'generating' : p.status)}</span>
      ...
      ${jobErr && !falDirect ? `<p class="fs-banner-err" role="alert">${esc(jobErr.error || 'A generation failed. Your script is still here.')}</p>` : ''}
      ...
      ${falDirect ? falStageHtml(p) : '<div id="fs-pipeline-slot" class="fs-pipeline-slot"></div>'}
      ${falDirect ? '' : `<section class="fs-thumb-stage">
        <h2>Thumbnail</h2>
  ...
  function bindHome() {
    ...
    if (main && $('fs-feed-grid')) bindFeedGrid(main)
    else if (main && $('fs-gallery')) bindShowcaseGrid(main, { onRecreate: (sample) => applyPreset(sample?.id) })
    if (main) {
      lazyVideos(main)
      playHeroFan(main)
    }
  ...
  function padComposer() {
    ...
    const form = $('fs-composer')
    if (home) home.classList.add('is-docked')
    if (form) {
      form.style.position = ''
      form.style.left = ''
      form.style.width = ''
  ...
  async function onGenerate(e) {
    ...
    if (composerBusy || falRun.active) return
    ...
    if (!fal && (!picked?.seconds || !picked.id)) {
    ...
    if (fal) {
      const cost = currentGenerateCredits()
      if (studioCredits.remaining != null && cost > studioCredits.remaining) {
        if (err) {
          err.hidden = false
          err.textContent = studioCredits.remaining <= 0
            ? 'You are out of Faceless Studio credits this month.'
            : ('This generate costs ' + cost + ' credits. You have ' + studioCredits.remaining + ' left.')
        }
        if (studioCredits.tier !== 'studio') window.openPaywall?.({ force: true, reason: 'upgrade' })
        return
      }
    }
    startGenCard()
    ...
    try {
      const payload = {
        ...
        aspect: compose.aspect,
        length: picked?.id || compose.length || 'long_180',
        duration_seconds: picked?.seconds || 180,
        voice_id: compose.voice,
        ...
        pipeline: {
          topic,
          aspect: compose.aspect,
          durationId: picked?.id || compose.length,
      ...
      created = true
      if (fal) {
        const count = clampVideoCount(compose.generations)
        const slots = []
        ...
        feed = slots.concat(feed)
        view = isTempChat() ? 'temp' : 'home'
        pendingFalVideo = false
        pendingAutoScript = false
        pushPath()
        renderMain()
      ...
      composerBusy = false
      const btn = $('fs-gen')
      if (btn && !falRun.active) btn.disabled = false
      if (!created) finishGenCard(false, err?.textContent || 'Could not create the project')
      else if (!fal) finishGenCard(true)
    }
  }
  ...
  function bindWorkspace() {
    ...
    $('fs-fal-again')?.addEventListener('click', () => queueFalVideo())
  ...
  function paintGenCard() {
    const host = $('fs-gen-host')
    if (!host) return
    if (!genCard.shown) {
      host.innerHTML = ''
      host.hidden = true
    ...
    host.hidden = false
    host.innerHTML = generationCardHtml({
      progress: genCard.progress,
      label: genCard.error && genCard.done ? 'failed' : 'generating',
      error: genCard.error && genCard.done ? genCard.error : '',
    })
  }
  ...
  function startGenCard() {
    ...
    genCard.done = false
    genCard.error = ''
    genCard.startedAt = Date.now()
    genCard.progress = Math.max(Number(genCard.progress) || 0, 3)
    paintGenCard()
    genTick = setInterval(() => {
      if (!genCard.shown || genCard.done) {
        clearInterval(genTick)
        genTick = null
        return
      }
      genCard.progress = Math.max(genCard.progress, generationProgressToward(Date.now() - genCard.startedAt))
      paintGenCard()
    }, 280)
  }

  function finishGenCard(ok, errText) {
    if (genTick) {
      clearInterval(genTick)
      genTick = null
    }
    if (!genCard.shown && ok) return
    genCard.shown = true
    genCard.done = true
    if (ok) {
      genCard.progress = 100
      genCard.error = ''
      paintGenCard()
      setTimeout(() => {
        if (!genCard.done || falRun.active || composerBusy) return
        genCard.shown = false
        genCard.progress = 0
        paintGenCard()
      }, 1200)
    } else {
      genCard.error = errText || genCard.error || 'Generation failed'
      paintGenCard()
    }
  }

  function setFalStatus(text, isErr) {
    falRun.label = text || ''
    const el = $('fs-fal-status')
    if (el) {
      el.hidden = !text
      el.textContent = text || ''
      el.classList.toggle('is-err', !!isErr)
    }
    const wait = document.querySelector('.fs-feed-wait')
    if (wait && text) wait.textContent = text
    const homeErr = $('fs-composer-error')
    if (homeErr && isErr && text) {
      homeErr.hidden = false
      homeErr.textContent = text
    }
  }

  async function queueFalVideo(existingSlots) {
    if (!current?.id || falRun.active) return
    const modelId = current.video_model || current.pipeline?.videoModel || compose.videoModel
    if (!isFalVideoModel(modelId)) {
      setFalStatus('Pick a Fal video model to generate.', true)
      finishGenCard(false, 'Pick a Fal video model to generate.')
      return
    }
    const prompt = String(current.topic || compose.topic || '').trim()
    if (!prompt) {
      setFalStatus('Enter a prompt first.', true)
      finishGenCard(false, 'Enter a prompt first.')
      return
    }
    const count = existingSlots?.length || clampVideoCount(current.video_count || current.pipeline?.generations || compose.generations)
    const duration = clipDurationFor(modelId, current.clip_duration || current.pipeline?.clipDuration || compose.clipDuration)
    const refs = (current.references || []).map((r) => r.url || r).filter((u) => /^https?:\/\//i.test(String(u)))
    ...
    if (!slots.length) {
      for (let i = 0; i < count; i++) {
        slots.push({
          ...
          pending: true,
          prompt,
          aspect: current.aspect || compose.aspect,
          model: modelId,
    ...
    startGenCard()
    falRun = { active: true, label: 'Starting ' + videoModelLabel(modelId) + '…', error: '' }
    current = { ...current, pipeline: { ...(current.pipeline || {}), falDirect: true }, status: 'running' }
    if (view === 'project') {
      view = isTempChat() ? 'temp' : 'home'
      pushPath()
    }
    if (!$('fs-feed-grid')) renderMain()
    else paintFeed()
    markRail()
    try {
      await studioReq('POST', '/projects/' + current.id + '/jobs', {
        ...
        progress: 5,
        provider: 'fal',
        parameters: { model: modelId, duration, aspect: current.aspect || compose.aspect, count },
      })
    } catch (_) {}
    const urls = []
    let lastErr = ''
    for (let i = 0; i < slots.length; i++) {
      setFalStatus('Generating ' + videoModelLabel(modelId) + (slots.length > 1 ? ' ' + (i + 1) + ' of ' + slots.length : '') + '…')
      genCard.progress = Math.max(genCard.progress, Math.round((i / Math.max(slots.length, 1)) * 82) + 8)
      paintGenCard()
      paintFeed()
      try {
        const data = await api.generate.video({
          ...
          aspect: current.aspect || compose.aspect,
          duration,
          generate_audio: !!(current.video_sound ?? current.pipeline?.sound ?? compose.sound),
          ...(current.video_resolution || compose.resolution ? { resolution: current.video_resolution || compose.resolution } : {}),
          ...(refs.length ? { image_urls: refs } : {}),
        })
        const url = data.urls?.[0]
        if (!url) throw new Error('No video came back')
        const added = await studioReq('POST', '/projects/' + current.id + '/assets', {
          type: 'broll',
          storage_url: url,
          mime: 'video/mp4',
          label: slots.length > 1 ? 'Clip ' + (i + 1) : videoModelLabel(modelId),
        })
        if (added?.project) current = added.project
        slots[i].pending = false
        slots[i].error = ''
        slots[i].url = url
        slots[i].id = added?.asset?.id || url
        urls.push(url)
        applyCreditsFromGenerate(data)
      } catch (e) {
        lastErr = e.message || 'Video generation failed'
        slots[i].pending = false
        slots[i].error = lastErr
        if (e.status === 402 && e.code === 'credits_exhausted') {
          lastErr = e.message || lastErr
          if (studioCredits.tier !== 'studio' && !window._fsPaywallOnce) {
            window._fsPaywallOnce = true
            window.openPaywall?.({ force: true, reason: 'upgrade' })
            setTimeout(() => { window._fsPaywallOnce = false }, 800)
          }
        } else if ((e.status === 403 || e.needsUpgrade || e.needsPlan) && !window._fsPaywallOnce) {
          window._fsPaywallOnce = true
          window.openPaywall?.({ force: true, reason: 'upgrade' })
          setTimeout(() => { window._fsPaywallOnce = false }, 800)
        }
    ...
    falRun.active = false
    falRun.error = urls.length ? '' : lastErr
    if (urls.length) finishGenCard(true)
    else finishGenCard(false, lastErr)
    try {
      await studioReq('POST', '/projects/' + current.id + '/jobs', {
        type: 'video',
        status: urls.length ? 'succeeded' : 'failed',
        progress: urls.length ? 100 : 0,
        error: falRun.error,
        provider: 'fal',
      })
      const patched = await studioReq('PATCH', '/projects/' + current.id, {
        status: urls.length ? 'ready' : 'failed',
        pipeline: { ...(current.pipeline || {}), falDirect: true, phase: 2 },
      })
      current = patched.project
    } catch (_) {
      try {
    ...
    if (!urls.length && lastErr) {
      falRun.error = lastErr
      setFalStatus(lastErr, true)
    }
    const btn = $('fs-gen')
    if (btn) btn.disabled = false
    paintFeed()
  ...
  async function go(next) {
    ...
    pushPath()
    markRail()
    if (next === 'generations') refreshJobs()
    else if (next === 'memory' || next === 'connectors' || next === 'templates') renderMain()
    else refreshProjects()
  }
  ...
  async function showFromPath() {
    ...
    markRail()
    if (view === 'generations' || tab === 'history') refreshJobs()
    else if (view === 'memory' || view === 'connectors' || view === 'temp' || view === 'templates') renderMain()
    else refreshProjects()
    } catch (e) {
  ...
  voiceBoot.finally(() => {
    ensureVoice()
    applyCreditsFromUser()
    if (view !== 'project') renderMain()
    else paintVideoBar()
  })
  try {
    api.user.me().then((u) => applyCreditsFromUser(u)).catch(() => {})
  } catch (_) {}
  showFromPath()
}
