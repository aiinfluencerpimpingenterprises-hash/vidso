...
export function initFacelessStudio(deps) {
  ...
  function pathForView() {
    ...
    if (isTempChat()) return '/faceless-studio/temp'
  ...
  function parsePath() {
    ...
    if (p === '/faceless-studio/temp') return { view: 'home', temp: true }
  ...
  function heroHtml() {
    return homeHeroHtml(isTempChat())
  }
  ...
  function renderMain() {
    ...
    if (view === 'projects' || view === 'favorites') {
      ...
      main.innerHTML = `<div class="fs-home has-composer">
        <div class="fs-tab-body">
          ${heroHtml()}
          ${composerHtml()}
          ${isTempChat() ? '' : homeTaglineHtml()}
          ${isTempChat() ? '' : galleryHtml()}
        </div>
        <div class="fs-composer-dock" id="fs-composer-dock" hidden></div>
      </div>`
    }
    bindMediaPlaceholders(main)
    bindHome()
  ...
  function markRail() {
    document.querySelectorAll('#fs-rail [data-fs-view]').forEach((el) => {
      ...
      el.classList.toggle('is-on', key === view || (view === 'home' && key === 'home' && !isTempChat()))
    })
    renderRailProjects()
    syncTempButton()
  }
  ...
  async function go(next) {
    ...
    window.fvStudioUnbind?.()
    current = null
    if (next === 'home') setTempChat(false)
    view = next
  ...
  async function showFromPath() {
    ...
    markRail()
    if (view === 'generations' || tab === 'history') refreshJobs()
    else if (view === 'memory' || view === 'connectors') renderMain()
    else refreshProjects()
    } catch (e) {
  ...
  $('fs-temp-chat')?.addEventListener('click', () => startTempChat())
