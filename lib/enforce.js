...
import { compedTier, planIsActive, withCompedPlan } from './comped.js'
...
export function resolveAccess(user, env) {
  ...
  const gifted = compedTier(granted)
  if (gifted) {
    return {
      active: true,
      tier: gifted,
      cycle: granted.plan_interval || (gifted === 'studio' ? 'yearly' : 'monthly'),
      legacy: false,
      planId: resolved.planId,
      entitlements: entitlementsFor(gifted),
    }
  }
...
function minTierForCredits(needed) {
  const want = Math.max(0, Number(needed) || 0)
  for (const tier of TIERS) {
    const n = Number(ENTITLEMENTS[tier].studio_credits_per_month) || 0
    if (n >= want) return tier
  }
  return 'studio'
}

/** Fal image/video credits. Independent of long-form / short-form video counts. */
export function evaluateStudioCredits({ user, cost, used = 0, env } = {}) {
  const plan = evaluatePlan(user, env)
  if (!plan.ok) return plan
  const charge = Math.max(1, Math.ceil(Number(cost) || 0))
  const limit = plan.legacy
    ? ENTITLEMENTS.plus.studio_credits_per_month
    : Number(plan.access.entitlements?.studio_credits_per_month) || 0
  const name = planDisplayName(plan.access?.tier || 'plus')
  const usedCount = Math.max(0, Math.floor(Number(used) || 0))
  const remaining = Math.max(0, limit - usedCount)
  if (charge > remaining) {
    const lift = minTierForCredits(usedCount + charge)
    const canUpgrade = lift && lift !== plan.access?.tier && (Number(ENTITLEMENTS[lift]?.studio_credits_per_month) || 0) > limit
    const wait = ' They reset on your billing anniversary.'
    const message = remaining <= 0
      ? (name + ' includes ' + limit + ' Faceless Studio credits per month.' +
        (canUpgrade ? ' Upgrade to ' + planDisplayName(lift) + ' (' + formatQuotaLabel(ENTITLEMENTS[lift].studio_credits_per_month) + ') to continue.' : wait))
      : ('This generation costs ' + charge + ' credits. You have ' + remaining + ' left this month.' +
        (canUpgrade ? ' Upgrade to ' + planDisplayName(lift) + ' for a larger allotment.' : wait))
    return denial(402, 'credits_exhausted', message, {
      limit,
      used: usedCount,
      remaining,
      charge,
      tier: plan.access?.tier,
      requiredTier: canUpgrade ? lift : undefined,
    })
  }
  return allow({
    access: plan.access,
