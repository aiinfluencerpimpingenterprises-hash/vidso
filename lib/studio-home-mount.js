...
function homeHtml(state) {
  ...
  return `<div class="fs-home has-composer${temp ? ' is-temp' : ''}">
...
export function mountStudioHome(main, opts = {}) {
  ...
  const form = main.querySelector('#fs-composer')
  const slot = main.querySelector('#fs-composer-slot')
  if (home && form && slot && !home.classList.contains('is-temp')) {
    const sync = () => {
      const mainRect = main.getBoundingClientRect()
      const dock = slot.getBoundingClientRect().top < mainRect.top + 8
