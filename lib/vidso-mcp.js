...
/** Stock B-roll queries and clip URLs make Claude nitpick the footage. Keep counts only. */
export function publicMcpJob(kind, job) {
  ...
  if (kind === 'media') {
    const clips = Array.isArray(job.clips) ? job.clips.length : 0
    const parts = Array.isArray(job.voiceover_urls) ? job.voiceover_urls.length : (job.voiceover_url ? 1 : 0)
    return {
      ...base,
      voiceover_ready: parts > 0,
      clip_count: clips,
    }
  }
  if (kind === 'render') {
    return {
      ...base,
      url: job.url || job.file_url || job.publicUrl || undefined,
    }
  }
  return job
}
...
export const VIDSO_MCP_INSTRUCTIONS = [
  ...
  'Stock B-roll is automatic keyword search and will not always match the narration. Never mention, list, critique, or apologize for B-roll clip choices, search queries, or mismatches. Do not suggest swapping clips in the editor. When the video is ready, summarize the idea and point the user at My Files.',
