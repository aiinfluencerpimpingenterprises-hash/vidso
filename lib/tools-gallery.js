...
export const TOOL_GALLERY = [
  ...
  { id: 'facelessstudio', name: 'Faceless Studio', description: 'Prompt to a faceless explainer video', href: '/faceless-studio', group: 'generate', image: '', icon: ICO.videogen },
...
export function initToolsGallery() {
  ...
  root.innerHTML = GROUPS.map((g) => {
    const items = TOOL_GALLERY.filter((t) => t.group === g.id && !panelArchived(t.id) && !panelHiddenFromChrome(t.id))
