 ...
 */
import { bindMediaPlaceholders } from '/lib/media-placeholder.js'

export const TOOL_SHOT_R2_HOST = 'pub-f40c956471ff49feab622906892ec527.r2.dev'

const ICO = {
  facelessstudio: '<svg fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M8 9h8M8 13h5"/></svg>',
...
export const TOOL_GALLERY = [
  { id: 'facelessstudio', name: 'Faceless Studio', description: 'Projects for script, voice, B-roll, and export', href: '/faceless-studio', group: 'generate', image: '', icon: ICO.facelessstudio },
...
function cardHtml(tool) {
  ...
  return `<a class="tool-shot" href="${tool.href}" data-go="${tool.id}">
    <div class="tool-shot-media media-ph" data-media-ph style="aspect-ratio:16 / 9">
      <div class="tool-shot-ph media-ph-icon" aria-hidden="true">${tool.icon}</div>
      ${src ? `<img class="tool-shot-img" src="${src}" alt="${tool.name}" width="640" height="360" loading="lazy" decoding="async" sizes="${SIZES}">` : ''}
    </div>
