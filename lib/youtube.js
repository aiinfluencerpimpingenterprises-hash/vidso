...
export function isYoutubeSidecarName(name) {
  const n = String(name || '')
  return n === YT_OAUTH_FILENAME || n.startsWith('vidso-yt-') || n === 'vidso-mcp.json'
}

export function youtubeOauthVerified(env = process.env) {
  const v = String(env.YOUTUBE_OAUTH_VERIFIED || '').trim().toLowerCase()
...
export function publicYoutubeStatus(record, req, extra = {}) {
  ...
  return {
    ...
    autoUpload: connected ? record.auto_upload === true : false,
    privacy: normalizePrivacy(record?.privacy),
    mcpUrl: origin + '/api/mcp',
    youtubeMcpUrl: origin + '/api/youtube/mcp',
    oauthVerified: extra.oauthVerified != null ? extra.oauthVerified : youtubeOauthVerified(),
    ...extra,
  }
}
...
export async function deleteYoutubeRecord(token, record) {
  if (record?.refresh_token) {
    try { await revokeGoogleToken(record.refresh_token) } catch (_) {}
  }
  if (record?._file_id) {
...
export function mcpTools() {
  return [
    {
      name: 'youtube_status',
      description: 'Show whether this Vidso account has a YouTube channel connected, plus auto-upload settings.',
      inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    },
    {
      name: 'youtube_connect_url',
      description: 'Tell the user how to connect a YouTube channel in Vidso (opens Connections).',
      inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    },
    {
      name: 'youtube_upload',
