...
function cardHtml(item, sizes, index) {
  ...
  const stats = item.views
    ? `<span class="results-meta">
        <span class="results-kicker">YouTube Studio</span>
        <span class="results-views">${esc(item.views)} <em>views</em></span>
        <span class="results-sub">
          <span>${esc(item.watchHours)} watch hours</span>
          <span>${esc(item.revenue)} est. revenue</span>
        </span>
      </span>`
    : ''
  return `
    <button type="button" class="results-card" data-result-id="${esc(item.id)}" aria-haspopup="dialog">
      <span class="results-shot">
        <span class="results-avatar-blur" aria-hidden="true"></span>
        <img
          src="${esc(item.src)}"
          alt="${esc(item.alt)}"
          width="${item.width}"
    ...
    <section class="results${multi ? ' is-multi' : ' is-single'}" id="results" aria-labelledby="results-heading">
      <div class="wrap">
        <div class="s-head center">
          ...
          <h2 id="results-heading">Channels that already <span class="accent">print</span></h2>
          <p>Live YouTube Studio dashboards. Not mockups.</p>
        </div>
