  ...
  return `<div class="fs-gallery" id="fs-gallery">
    <div class="fs-g-grid fs-shot-grid">
      ${STUDIO_SAMPLES.map((s, i) => `<article class="fs-shot" data-preset="${esc(s.id)}" data-shot-index="${i}">
        <video class="fs-shot-vid" muted loop playsinline preload="metadata" data-src="${esc(s.video)}" aria-hidden="true"></video>
...
/**
 * Attach video sources only once a tile is near the viewport. Ten templates all
 * pulling metadata on mount stalled the page before anything was visible.
 */
export function lazyVideos(root) {
  const pending = Array.from((root || document).querySelectorAll('video[data-src]'))
  if (!pending.length) return
  if (typeof IntersectionObserver !== 'function') {
    pending.forEach(loadLazyVideo)
    return
  }
  // One shared observer: the feed repaints while clips generate, and a new
  // observer per repaint would pile up watchers on tiles already loaded.
  if (!lazyObserver) {
  ...
  for (const v of pending) {
    if (v.dataset.lazyWatched === '1') continue
    v.dataset.lazyWatched = '1'
...
function thumbsHtml() {
  return STUDIO_SAMPLES.map((s, i) =>
    `<button type="button" class="fs-show-thumb${i === index ? ' is-on' : ''}" data-shot-index="${i}" aria-label="${esc(s.name)}">
      <video muted playsinline preload="none" data-src="${esc(s.video)}" tabindex="-1"></video>
    </button>`,
  ).join('')
}
...
function paint() {
  ...
  if (thumbs) {
    // Built once; stepping through samples only moves the highlight, because
    // rebuilding the strip recreated every thumbnail video on each arrow press.
    if (thumbs.dataset.fsBuilt !== '1') {
      thumbs.innerHTML = thumbsHtml()
      thumbs.dataset.fsBuilt = '1'
      lazyVideos(thumbs)
    }
    thumbs.querySelectorAll('.fs-show-thumb').forEach((btn, i) => {
...
export function bindShowcaseGrid(root, opts = {}) {
  ...
  bindShowcaseOnce()
  if (!root) return
  const grid = root.querySelector('#fs-gallery .fs-shot-grid')
  if (!grid) return
  lazyVideos(grid)
  ...
  grid.addEventListener('pointerover', (e) => {
    const card = e.target.closest?.('.fs-shot')
    if (!card || card.contains(e.relatedTarget)) return
    const vid = card.querySelector('video')
    if (!vid) return
    const src = vid.getAttribute('data-src')
    if (src) { vid.removeAttribute('data-src'); vid.src = src }
  ...
  grid.addEventListener('click', (e) => {
    ...
    if (rec) {
      e.preventDefault()
      e.stopPropagation()
      recreate(rec.getAttribute('data-fs-recreate'))
      return
    }
    const card = e.target.closest('.fs-shot')
    if (card) openStudioShowcase(card.getAttribute('data-preset'))
  })
}
