...
import { boundBrollKeywords, brollScriptBrief } from './broll-match.js'
...
/** Railway media/TTS rejects a script over 8000 characters. Long-form
 *  narration is split into chunks under this cap, then stitched. */
export const RAILWAY_MEDIA_SCRIPT_MAX = 8000
/** Cap per media job so Railway TTS does not need /api/media/concat (missing on prod). */
export const RAILWAY_MEDIA_SCRIPT_SAFE = 4500
export const RAILWAY_MEDIA_SECTION_SNIPPET = 400
...
function splitPlainText(text, maxChars) {
  const raw = String(text || '').trim()
  const cap = Math.max(200, Number(maxChars) || RAILWAY_MEDIA_SCRIPT_SAFE)
  if (!raw) return []
  ...
  function take(piece) {
    ...
    for (const sentence of sentences) {
      ...
      if (!bit) continue
      if ((buf ? buf.length + 1 + bit.length : bit.length) <= cap) {
        buf = buf ? buf + ' ' + bit : bit
        continue
      }
      pushBuf()
      ...
      for (let i = 0; i < bit.length; i += cap) parts.push(bit.slice(i, i + cap))
    }
  }
...
export function splitScriptForMedia(script, maxChars = RAILWAY_MEDIA_SCRIPT_SAFE) {
  const cap = Math.max(200, Number(maxChars) || RAILWAY_MEDIA_SCRIPT_SAFE)
  ...
  const sections = Array.isArray(src.sections) && src.sections.length
    ...
    : [{
      id: 's1',
      heading: '',
      text: String(src.full_script || ''),
    }]
  const chunks = []
  let cur = []
  let curLen = 0

  function asChunk(list) {
    const full_script = list.map((s) => s.text).filter(Boolean).join('\n\n')
    return {
      title: src.title,
      topic: src.topic,
      keywords: boundBrollKeywords(src.keywords || [], { topic: src.topic, sections: list, stock: true }),
      aspect: src.aspect,
      voice_id: src.voice_id,
      sections: list.map((s) => ({
        id: s.id,
        heading: s.heading,
        text: String(s.text || '').slice(0, RAILWAY_MEDIA_SECTION_SNIPPET),
      })),
      full_script,
    }
  }
  ...
  for (const sec of sections) {
    ...
    if (text.length > cap) {
      flush()
      for (const part of splitPlainText(text, cap)) {
        chunks.push(asChunk([{ ...sec, text: part }]))
      }
      continue
    }
    const add = curLen ? 2 + text.length : text.length
    if (cur.length && curLen + add > cap) flush()
    cur.push(sec)
    curLen += add
  }
  flush()
  return chunks.length ? chunks : [asChunk([{ id: 's1', heading: '', text: '' }])]
}
...
export function mergeMediaJobs(jobs) {
  ...
  for (const job of list) {
    ...
    if (job.voiceover_url) voiceover_urls.push(job.voiceover_url)
    ...
    for (const c of job.clips || []) clips.push(c)
    for (const seg of job.timeline || []) {
      timeline.push({
        ...seg,
  ...
  return {
    ...last,
    voiceover_url: voiceover_urls[0] || last.voiceover_url || '',
    voiceover_urls,
    duration: offset,
    words,
    clips,
