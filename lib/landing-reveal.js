...
export function checkHeadingCenters(root = document) {
  ...
  const secs = [...root.querySelectorAll('.showcase, .shorts, .workflow-tiles, .mcp-hero, .formats-float, .why-stack, .faq, .final, .format-econ, .compare, .comp-compare, .pricing, .results')]
...
export function mountLandingReveals() {
  ...
  const scopes = [...page.querySelectorAll('.showcase, .shorts, .workflow-tiles, .lp-feat, .mcp-hero, .formats-float, .why-stack, .faq, .final, .format-econ, .compare, .comp-compare, .pricing, .results')]
  scopes.forEach((sec) => {
    const eyebrow = sec.querySelector('.s-head .eyebrow, .shorts-head .eyebrow')
    if (eyebrow) eyebrow.classList.add('rv-eye')
    sec.querySelectorAll('h2.two-line, h2.h-split, .shorts-title, .mcp-title, .s-head h2, .final-box h2').forEach(wrapLines)
