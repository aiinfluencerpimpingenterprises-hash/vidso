...
export function checkHeadingCenters(root = document) {
  const skip = (el) => !!(
    ...
    el.closest('.studio-pill') ||
    el.closest('.yt-stat') ||
    el.closest('.ugc-card')
  )
  const secs = [...root.querySelectorAll('.showcase, .shorts, .workflow-tiles, .top-models, .models-hero, .models-index, .mcp-hero, .mcp-excerpt, .mcp-setup, .mcp-bring, .mcp-ask, .mcp-models, .mcp-clients, .formats-float, .why-stack, .faq, .final, .format-econ, .compare, .comp-compare, .pricing, .results, .yt-compact, .ugc-row, .studio-tools')].filter((sec) => !sec.hasAttribute('data-roadmap'))
...
export function mountLandingReveals() {
  ...
  const scopes = [...page.querySelectorAll('.showcase, .shorts, .workflow-tiles, .top-models, .models-hero, .models-index, .lp-feat, .mcp-hero, .mcp-excerpt, .mcp-setup, .mcp-bring, .mcp-ask, .mcp-models, .mcp-clients, .demo-hero, .formats-float, .why-stack, .faq, .final, .format-econ, .compare, .comp-compare, .pricing, .results, .yt-compact, .ugc-row, .studio-tools')].filter((sec) => !sec.hasAttribute('data-roadmap') && !sec.hidden)
  scopes.forEach((sec) => {
    ...
    sec.querySelectorAll('.s-head .lp-sub, .shorts-head .lp-sub, .mcp-sub, .final-box p, .wf-punch, .formats-float .lp-sub, .mcp-excerpt .lp-sub, .mcp-bring .lp-sub, .mcp-ask .lp-sub, .mcp-models .lp-sub, .top-models-copy .lp-sub, .models-hero .lp-sub, .yt-compact .lp-sub, .ugc-row .lp-sub, .studio-tools .lp-sub').forEach((p) => p.classList.add('rv-sub'))
    sec.querySelectorAll('.feat-media, .coverflow, .shorts-stage, .results-shot, .tab-still, .mcp-keys, .mcp-clients-card, .demo-frame, .comp-table-wrap, .final-box, .top-models-orbit, .models-grid, .studio-panel, .ugc-row-track, .yt-stats').forEach((m) => m.classList.add('rv-media'))
    const itemSel = document.body.classList.contains('is-mcp-page')
      ? '.wf-tile, .fmt-float, .cmp, .plan, .yt-stat, .ugc-card'
      : '.wf-tile, .fmt-float, .cmp, .plan, .faq-grid details, .yt-stat, .ugc-card'
