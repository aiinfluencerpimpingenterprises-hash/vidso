...
export function checkHeadingCenters(root = document) {
  ...
  const secs = [...root.querySelectorAll('.showcase, .shorts, .workflow-tiles, .mcp-hero, .mcp-excerpt, .mcp-setup, .mcp-bring, .mcp-ask, .mcp-clients, .formats-float, .why-stack, .faq, .final, .format-econ, .compare, .comp-compare, .pricing, .results')]
...
export function mountLandingReveals() {
  ...
  const scopes = [...page.querySelectorAll('.showcase, .shorts, .workflow-tiles, .lp-feat, .mcp-hero, .mcp-excerpt, .mcp-setup, .mcp-bring, .mcp-ask, .mcp-clients, .demo-hero, .formats-float, .why-stack, .faq, .final, .format-econ, .compare, .comp-compare, .pricing, .results')]
  scopes.forEach((sec) => {
    const eyebrow = sec.querySelector('.s-head .eyebrow, .shorts-head .eyebrow')
    if (eyebrow) eyebrow.classList.add('rv-eye')
    sec.querySelectorAll('h2.two-line, h2.h-split, .shorts-title, .s-head h2, .final-box h2').forEach(wrapLines)
    sec.querySelectorAll('.s-head .lp-sub, .shorts-head .lp-sub, .mcp-sub, .final-box p, .wf-punch, .formats-float .lp-sub, .mcp-excerpt .lp-sub, .mcp-bring .lp-sub, .mcp-ask .lp-sub').forEach((p) => p.classList.add('rv-sub'))
    sec.querySelectorAll('.feat-media, .coverflow, .shorts-stage, .results-shot, .tab-still, .mcp-panel, .mcp-keys, .mcp-blank, .mcp-clients-card, .demo-frame, .comp-table-wrap, .final-box').forEach((m) => m.classList.add('rv-media'))
