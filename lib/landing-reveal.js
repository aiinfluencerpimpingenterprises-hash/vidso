...
export function mountLandingReveals() {
  ...
  scopes.forEach((sec) => {
    ...
    sec.querySelectorAll('.s-head .lp-sub, .shorts-head .lp-sub, .mcp-sub, .final-box p, .wf-punch, .formats-float .lp-sub, .mcp-excerpt .lp-sub, .mcp-bring .lp-sub, .mcp-ask .lp-sub, .top-models-copy .lp-sub, .models-hero .lp-sub, .yt-compact .lp-sub, .ugc-row .lp-sub, .studio-tools .lp-sub').forEach((p) => p.classList.add('rv-sub'))
    sec.querySelectorAll('.feat-media, .coverflow, .shorts-stage, .results-shot, .tab-still, .mcp-panel, .mcp-keys, .mcp-blank, .mcp-clients-card, .demo-frame, .comp-table-wrap, .final-box, .top-models-orbit, .models-grid, .studio-panel, .ugc-row-track, .yt-stats').forEach((m) => m.classList.add('rv-media'))
    const items = sec.querySelectorAll('.wf-tile, .fmt-float, .mcp-step, .cmp, .plan, .faq-grid details, .yt-stat, .ugc-card')
    items.forEach((el, i) => {
