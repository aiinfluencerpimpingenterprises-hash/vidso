...
export function checkHeadingCenters(root = document) {
  ...
  const secs = [...root.querySelectorAll('.loved, .showcase, .shorts, .workflow-tiles, .mcp-hero, .formats-float, .why-stack, .faq, .final, .format-econ, .compare, .comp-compare, .pricing, .results')]
...
export function mountLandingReveals() {
  ...
  const scopes = [...page.querySelectorAll('.loved, .showcase, .shorts, .workflow-tiles, .lp-feat, .mcp-hero, .formats-float, .why-stack, .faq, .final, .format-econ, .compare, .comp-compare, .pricing, .results')]
  scopes.forEach((sec) => {
    const eyebrow = sec.querySelector('.s-head .eyebrow, .shorts-head .eyebrow')
    if (eyebrow) eyebrow.classList.add('rv-eye')
    sec.querySelectorAll('h2.two-line, h2.h-split, .shorts-title, .mcp-title, .s-head h2, .final-box h2, .loved h2').forEach(wrapLines)
    sec.querySelectorAll('.s-head .lp-sub, .shorts-head .lp-sub, .mcp-sub, .final-box p, .wf-punch, .formats-float .lp-sub').forEach((p) => p.classList.add('rv-sub'))
    sec.querySelectorAll('.feat-media, .coverflow, .shorts-stage, .results-shot, .tab-still, .mcp-chat, .mcp-panel, .mcp-keys, .comp-table-wrap, .final-box').forEach((m) => m.classList.add('rv-media'))
