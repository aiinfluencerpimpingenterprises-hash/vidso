...
export function catalogHasClip(id) {
  const slug = modelSlug(id)
  return Boolean(CATALOG_CLIP_FILES[id] || CATALOG_CLIP_FILES[slug])
}
...
/** Skip or cut a named clip. end 0 means play through to the file end. */
export const CATALOG_CLIP_WINDOWS = {
  'seedream-5-pro': { start: 0, end: 5 },
  'nano-banana-2-5': { start: 2.05, end: 0 },
}

export function catalogClipWindow(id) {
  const slug = modelSlug(id)
  const row = CATALOG_CLIP_WINDOWS[id] || CATALOG_CLIP_WINDOWS[slug]
  if (!row) return null
  const start = Math.max(0, Number(row.start) || 0)
  const end = Math.max(0, Number(row.end) || 0)
  if (!(start > 0 || end > 0)) return null
  return { start, end }
}

export function catalogClipLoopSeconds(id) {
  const win = catalogClipWindow(id)
  if (!win || !(win.end > win.start)) return 0
  return win.start > 0 ? 0 : win.end
}

export function bindCatalogClipLoop(video, secondsOrWindow) {
  const win = typeof secondsOrWindow === 'number'
    ? (secondsOrWindow > 0 ? { start: 0, end: secondsOrWindow } : null)
    : secondsOrWindow
  if (!video || !win) return
  const start = Math.max(0, Number(win.start) || 0)
  const end = Math.max(0, Number(win.end) || 0)
  video.loop = false
  const play = () => {
    try {
      const p = video.play()
  ...
  const restart = () => {
    const pastEnd = end > 0 && video.currentTime >= end - 0.04
    const before = start > 0 && video.currentTime + 0.08 < start
    if (!pastEnd && !before) return
    seekStart()
    play()
  }
...
const NEW_IDS = new Set(['seedream-5-pro'])
...
export const CATALOG_MODELS = CATALOG_MODEL_ROWS.filter((m) => catalogHasClip(m.id))
...
/** Landing orbit uses the /models card clip for the same model, or the closest catalog sibling. */
const FEATURED_MEDIA_ID = {
  'nano-banana-pro': 'nano-banana-2-5',
  'flux-2-pro': 'flux-3',
}

export function featuredMediaId(id) {
  return FEATURED_MEDIA_ID[id] || id
}

export function landingFeaturedModels() {
