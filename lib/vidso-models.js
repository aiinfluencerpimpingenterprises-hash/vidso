...
export function placeholderTable() {
  return [
    ...
    ...CATALOG_MODELS.map((m) => ({ file: 'mcp-model-' + modelSlug(m.id) + '.jpg', place: '/mcp models tile · ' + m.name })),
    ...CATALOG_MODELS.map((m) => ({ file: 'mcp-model-' + modelSlug(m.id) + '.mp4', place: '/mcp models hover clip · ' + m.name })),
  ]
}
