...
export function mcpModelTileSrc(id) {
  return R2 + '/mcp-model-' + modelSlug(id) + '.jpg'
}

export function mcpModelClipSrc(id) {
  return R2 + '/mcp-model-' + modelSlug(id) + '.mp4'
}
...
export function placeholderTable() {
  return [
    ...
    ...mcpLatestModels().map((m) => ({ file: 'mcp-model-' + modelSlug(m.id) + '.jpg', place: '/mcp models tile · ' + m.name })),
    ...mcpLatestModels().map((m) => ({ file: 'mcp-model-' + modelSlug(m.id) + '.mp4', place: '/mcp models hover clip · ' + m.name })),
  ]
}
