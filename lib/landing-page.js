...
import { mountLandingReveals, checkHeadingCenters } from './landing-reveal.js?v=d1f20'
  ...
  afterIdle(() => {
    probeUrl(poster).then((okPoster) => {
      if (okPoster) {
        ...
        host.closest('.hero-card, .hero-mq-card')?.classList.remove('is-blank')
        fadeMedia(img)
      }
      if (!wantVideo) return
...
function createHref() {
  try {
    if (localStorage.getItem('clipzo_token')) return GENERATE_HREF
  } catch (_) {}
  return CREATE_HREF
}
...
function mountHeroTransition() {
  ...
  const exits = {
    ...
    prompt: document.querySelector('[data-hero-exit="prompt"]'),
    cards: document.querySelector('[data-hero-exit="cards"]'),
    trust: document.querySelector('[data-hero-exit="trust"]'),
  }
  ...
  function apply(p) {
    ...
    if (p <= 0.002) {
      ...
      if (heroLp) {
        heroLp.style.opacity = ''
        heroLp.style.visibility = ''
      }
    ...
    const titleT = range(a, 0, 0.48)
    const copyT = range(a, 0.04, 0.55)
    const ctaT = range(a, 0.08, 0.58)
    const promptT = range(a, 0.28, 0.78)
    const rowT = range(a, 0.42, 1)
    fadeEl(exits.title, 1 - titleT, -48 * titleT)
    fadeEl(exits.badge, 1 - copyT, -28 * copyT)
    fadeEl(exits.sub, 1 - copyT, -36 * copyT)
    fadeEl(exits.cta, 1 - ctaT, -24 * ctaT)
    fadeEl(exits.prompt, 1 - promptT, -16 * promptT)
    fadeEl(exits.trust, 1 - copyT, -20 * copyT)
    fadeEl(exits.cards, 1 - rowT, 36 * rowT)
    if (p >= aEnd) {
      fadeEl(exits.title, 0, -40)
      fadeEl(exits.badge, 0, -28)
      fadeEl(exits.sub, 0, -36)
      fadeEl(exits.cta, 0, -24)
      fadeEl(exits.prompt, 0, -16)
      fadeEl(exits.cards, 0, 40)
      fadeEl(exits.trust, 0, -20)
    }
    const heroLp = document.querySelector('.hero-lp')
    if (heroLp) {
      heroLp.style.opacity = p >= aEnd ? '0' : ''
      heroLp.style.visibility = p >= aEnd ? 'hidden' : ''
    }

    const beatPeak = b < 0.5 ? b * 2 : (1 - b) * 2
    if (beat) beat.style.opacity = String(p < aEnd ? 0 : (p > bEnd ? Math.max(0, 1 - c * 2) * 0.18 : beatPeak * 0.55))
  ...
  function render() {
    ...
    SHOWCASE_CARDS.forEach((c, i) => {
      ...
      el.innerHTML = `<span class="cover-badge">${c.category}</span>${vid}${c.src ? muteBtn() : ''}
        <div class="cover-prompt"><p>${c.prompt}</p><a class="btn btn-primary btn-glow" href="${CREATE_HREF}"><span class="btn-ring" aria-hidden="true"></span><span class="btn-label">Create Now</span></a></div>`
...
function wrapGlowLabels(root = document) {
