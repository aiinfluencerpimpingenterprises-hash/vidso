...
import { mountLandingReveals, checkHeadingCenters } from './landing-reveal.js?v=d1f52'
  ...
  afterIdle(() => {
    probeUrl(poster).then((okPoster) => {
      if (okPoster) {
        ...
        host.closest('.hero-card, .hero-mq-card, .top-models-tile, .ugc-card, .studio-shot, .feat-media')?.classList.remove('is-blank')
        fadeMedia(img)
      }
      if (!wantVideo) return
...
export function mountFaq(root = document) {
  ...
  grids.forEach((grid) => {
    ...
    else grid.querySelectorAll('.faq-col').forEach((col) => bindFaqGroup(col))
  })
}
...
function mountUgc() {
  ...
  row.innerHTML = UGC_CARDS.map((c) => `
    <article class="ugc-card is-blank">
      <div class="ugc-shot is-blank" data-poster="${c.poster}" data-src="${c.src}"></div>
    </article>`).join('')
...
function mountStudioTools() {
  const pills = document.getElementById('studio-pills')
  const media = document.getElementById('studio-media')
  if (!pills || !media) return
  pills.innerHTML = STUDIO_TOOLS.map((t, i) => `
    <button type="button" class="studio-pill${i === 0 ? ' is-on' : ''}" role="tab" aria-selected="${i === 0 ? 'true' : 'false'}" data-studio="${t.id}">
      ${t.name}
    </button>`).join('')
  function show(id) {
    const tool = STUDIO_TOOLS.find((t) => t.id === id) || STUDIO_TOOLS[0]
    pills.querySelectorAll('.studio-pill').forEach((b) => {
      const on = b.getAttribute('data-studio') === tool.id
      b.classList.toggle('is-on', on)
      b.setAttribute('aria-selected', on ? 'true' : 'false')
    })
    media.innerHTML = `<div class="studio-shot is-blank"></div><p class="studio-cap">${tool.name}</p>`
    attachOptionalMedia(media.querySelector('.studio-shot'), tool.src, '', { lazy: true, width: 720, height: 405 })
  }
  pills.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-studio]')
    if (btn) show(btn.getAttribute('data-studio'))
  })
  show(STUDIO_TOOLS[0].id)
}
...
export function mountLandingPage() {
  ...
  mountAnnounce()
  mountNavShrink()
  if (!document.body.classList.contains('is-home')) mountLandingToolsMenu()
  mountLandingFooter()
  try {
