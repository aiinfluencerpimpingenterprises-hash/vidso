...
import { mountLandingReveals } from './landing-reveal.js'
  ...
  afterIdle(() => {
    probeUrl(poster).then((okPoster) => {
      if (okPoster) {
        ...
        host.closest('.hero-card')?.classList.remove('is-blank')
        fadeMedia(img)
      }
      if (!wantVideo) return
...
function mountShorts() {
  ...
  function layout() {
    ...
    items.forEach((el, i) => {
      ...
      if (off > n / 2) off -= n
      if (off < -n / 2) off += n
      const rotY = clamp(off * (compact ? 16 : 14), -28, 28)
      const drop = Math.abs(off) * (compact ? 14 : 22)
      const z = -Math.abs(off) * 48
      el.style.width = cardW + 'px'
      el.style.marginLeft = (-cardW / 2) + 'px'
      el.style.transform = `translate3d(${off * step}px, ${drop}px, 0)`
...
function announceHeight() {
  ...
  return bar.offsetHeight || 42
}

function applyAnnounceOffset() {
  const h = announceHeight()
  const header = document.querySelector('.lp-red header')
  const navH = header ? header.offsetHeight : 64
  document.documentElement.style.setProperty('--announce-h', h + 'px')
  document.documentElement.style.setProperty('--anchor-offset', (navH + h + 16) + 'px')
