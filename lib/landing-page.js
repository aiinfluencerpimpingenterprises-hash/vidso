...
import {
  ...
  UGC_CARDS,
  WORKFLOW_TILES,
} from './landing-capability.js?v=d1fb8'
...
function revealHeroMedia(host, el) {
  host.classList.remove('is-blank')
  el.classList.remove('is-blank')
}

function mountHeroLiveMedia(host, el, src) {
  if (!host || !src) return
  if (isImageSrc(src)) {
    const img = document.createElement('img')
    img.alt = ''
    ...
    img.addEventListener('load', () => {
      revealHeroMedia(host, el)
      fadeMedia(img)
    })
    img.addEventListener('error', () => { try { img.remove() } catch (_) {} })
    host.appendChild(img)
    return
  }
  const v = document.createElement('video')
...
function bindHeroCard(el) {
  ...
  if (src && !/\/landing\/hero-card-\d\d\.mp4$/.test(src)) {
    mountHeroLiveMedia(host, el, src)
  } else {
  ...
  const unmute = () => {
    ...
    if (!v) return
    if (audioOwner && audioOwner !== v) audioOwner.muted = true
    v.muted = false
    audioOwner = v
    try {
  ...
  el.addEventListener('pointerenter', unmute)
...
function attachFormatClip(host, src) {
  if (!host || !src) return
  const v = document.createElement('video')
  v.muted = true
  v.defaultMuted = true
...
function mountShorts() {
  ...
  drawShortsArc()
  const liveFormatSrc = {
    'Long-form': 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/landingpagelongform.mp4',
    Shorts: 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/landingpageformatsshort.mp4',
    Ads: 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/landingpageformatsad.mp4',
    UGC: 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/landingpageformatsugc.mp4',
    Explainers: 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/facelessstudio3.mp4',
    'Product videos': 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/landingpageformatsproduct.mp4',
    'Talking head': 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/landingpageformatstalkinghead.mp4',
  }
  const cards = SHORTS_CARDS.filter((c) => c.slug !== 'listicles' && c.slug !== 'documentary')
  row.innerHTML = cards.map((c, i) => {
    const icon = IDEA_ICONS[c.icon] || IDEA_ICONS.film
    const src = liveFormatSrc[c.category] || c.src
    return `<article class="shorts-card" data-short="${i}" tabindex="0" aria-label="${c.category}">
      <div class="shorts-3d">
        <div class="shorts-shot is-blank" data-poster="${c.poster}" data-src="${src}"></div>
      </div>
  ...
  items.forEach((el) => {
    ...
    if (src && !/\/landing\/format-card-/.test(src)) {
      attachFormatClip(host, src)
      return
    }
    attachOptionalMedia(host, host.getAttribute('data-poster'), src, {
      video: false,
      lazy: true,
      width: 220,
      height: 390,
  ...
  document.querySelector('[data-short-prev]')?.addEventListener('click', () => go(idx - 1))
  document.querySelector('[data-short-next]')?.addEventListener('click', () => go(idx + 1))
...
function mountFeatMedia() {
  const liveFeatSrc = {
    editor: 'https://pub-f40c956471ff49feab622906892ec527.r2.dev/videoeditor.png',
  }
  document.querySelectorAll('[data-feat-media]').forEach((host) => {
    if (host.querySelector('.vo-box, video, img')) return
    const live = liveFeatSrc[host.getAttribute('data-feat-media')]
    if (live) {
      const img = document.createElement('img')
      img.alt = ''
      img.decoding = 'async'
...
function attachTopModelClip(host, tile, mediaId) {
  ...
  v.defaultMuted = true
  v.volume = 0
  v.loop = !catalogClipWindow(mediaId)
  v.playsInline = true
  v.autoplay = true
  v.preload = 'metadata'
  v.setAttribute('muted', '')
  ...
  v.setAttribute('type', 'video/mp4')
  v.src = clip
  bindCatalogClipLoop(v, catalogClipWindow(mediaId))
  v.addEventListener('loadeddata', () => {
    host.classList.remove('is-blank')
    tile.classList.remove('is-blank')
    if (reduceMotion()) return
