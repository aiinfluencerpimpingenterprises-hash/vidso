...
/**
 * Live panels that stay off the chrome: no top-nav item, no search hit, no
 * tools card. The route still works so they can be opened by URL.
 */
export const HIDDEN_CHROME_PANELS = []

export function panelHiddenFromChrome(id) {
  return HIDDEN_CHROME_PANELS.includes(id)
}

/**
 * Panels that ship but are not reachable: no nav entry, no tool card, and the
 * route falls back to the default panel. Adding ?studio=1 to any dashboard URL
 * opens them for the rest of the tab session, so they stay testable on prod.
 *
 * To un-archive: empty this list and drop the matching `data-archived hidden`
 * attributes from the nav entries in dashboard/index.html.
 */
export const ARCHIVED_PANELS = ['facelessstudio']

const ARCHIVE_PREVIEW_KEY = 'vidso_archive_preview'

export function archivePreviewOn() {
  try {
    if (new URLSearchParams(location.search).get('studio') === '1') {
      sessionStorage.setItem(ARCHIVE_PREVIEW_KEY, '1')
      return true
    }
    return sessionStorage.getItem(ARCHIVE_PREVIEW_KEY) === '1'
  } catch (_) {
    return false
  }
}

export function panelArchived(id) {
  if (!ARCHIVED_PANELS.includes(id)) return false
  return !archivePreviewOn()
}

/**
 * Archived entries ship hidden in the markup. Take them out of the DOM entirely
 * so nothing can focus or click them, or reveal them when previewing.
 */
export function removeArchivedNav() {
  const preview = archivePreviewOn()
  document.querySelectorAll('[data-archived]').forEach((el) => {
    if (preview) el.removeAttribute('hidden')
    else el.remove()
  })
}
...
export const NAV_SEARCH_ITEMS = [
  ...
  { id: 'facelessstudio', label: 'Faceless Studio' },
...
export function initBanner() {
  ...
  if (msg) {
    ...
    const mark = 'Seedream 5.0 Pro'
    const node = msg.firstChild
    if (node && node.nodeType === 3 && node.textContent.startsWith(mark)) {
      const name = document.createElement('strong')
      name.className = 'app-banner-name'
      name.textContent = mark
      node.textContent = node.textContent.slice(mark.length)
      msg.insertBefore(name, node)
    }
  }
  if (cta) {
...
function renderSearchList(q) {
  ...
  const items = NAV_SEARCH_ITEMS
    .filter((it) => !panelArchived(it.id) && !panelHiddenFromChrome(it.id))
    .filter((it) => !needle || it.label.toLowerCase().includes(needle))
