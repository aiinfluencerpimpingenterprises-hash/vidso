...
export const TOOL_PANELS = ['captions', 'voiceover', 'reframe', 'editor', 'downloader', 'commentary']
...
export function initTopNav() {
  applyNavBadges()
  bindDrop(document.getElementById('nav-tools-wrap'))
  ...
  document.getElementById('nav-tools-menu')?.addEventListener('click', () => {
    closeNavMenus()
    window.closeMobileNav?.()
  })
...
export function markNavActive(panelId) {
  ...
  const toolsOn = TOOL_PANELS.includes(panelId) || panelId === 'tools'
