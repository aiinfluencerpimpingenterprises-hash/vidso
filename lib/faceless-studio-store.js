...
import { fetchJsonUrl, fetchJsonUrls, railwayDelete, railwayList, railwayUpload } from './railway-files.js'
...
export async function listProjects(token, user, { q = '', sort = 'updated', favorites = false, visibility = '', offset = 0, limit = 24 } = {}) {
  ...
  const records = await fetchJsonUrls(metas.map((f) => f.url))
  const matched = []
  metas.forEach((file, i) => {
    const rec = records[i]
    if (!rec?.id) return
    if (!projectVisibleTo(rec, user)) return
    rec.meta_file_id = rec.meta_file_id || file.id
    rec._meta_file = file
    if (favorites && !rec.favorited) return
