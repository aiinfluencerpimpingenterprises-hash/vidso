import { mountAnnounce, mountNavShrink, wrapGlowLabels } from './landing-page.js?v=d1fa9'
...
function attachClip(host, url, loopSeconds) {
  ...
  v.addEventListener('loadeddata', () => {
    const card = host.closest('.model-card')
    if (card) markFilled(card, host)
    else markFilled(host, host)
...
function bindCardMedia(card, model) {
  ...
  if (!catalogHasClip(model.id)) return
  attachClip(host, clip, catalogClipLoopSeconds(model.id))
}

function cardHtml(model) {
  const slug = modelSlug(model.id)
  return `<article class="model-card is-blank" id="${slug}" data-group="${model.group}" data-model="${slug}">
...
function paintGrid() {
  ...
  const io = new IntersectionObserver(playVisible, { threshold: 0.12 })
  grid.querySelectorAll('.model-card').forEach((card) => io.observe(card))
}
