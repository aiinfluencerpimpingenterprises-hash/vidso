...
import { mountLandingReveals } from './landing-reveal.js?v=d1f40'
...
function cardHtml(model) {
  ...
  return `<article class="model-card is-blank" id="${slug}" data-group="${model.group}" data-model="${slug}" tabindex="0">
    ...
    <span class="model-card-go" aria-hidden="true">
...
function paintGrid(filter) {
  ...
  if (!grid) return
  const list = filter === 'all' ? VIDSO_MODELS : VIDSO_MODELS.filter((m) => m.group === filter)
  grid.innerHTML = list.map(cardHtml).join('')
  grid.querySelectorAll('.model-card').forEach((card) => {
    const model = VIDSO_MODELS.find((m) => modelSlug(m.id) === card.id)
...
function mountFilters() {
  const row = document.getElementById('models-filters')
  if (!row) return
  row.innerHTML = MODEL_FILTERS.map((f, i) =>
    `<button type="button" class="models-filter${i === 0 ? ' is-on' : ''}" data-filter="${f.id}" aria-pressed="${i === 0 ? 'true' : 'false'}">${f.label}</button>`
  ).join('')
  row.addEventListener('click', (e) => {
    const btn = e.target.closest('[data-filter]')
    if (!btn) return
    const id = btn.getAttribute('data-filter')
    row.querySelectorAll('.models-filter').forEach((b) => {
      const on = b === btn
