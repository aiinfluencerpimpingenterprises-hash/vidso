...
import { evaluateFeature, evaluateGeneration, evaluateLength, evaluatePlan, toHttp } from './enforce.js'
...
export function ruleFor(method, subpath) {
  ...
  if (method === 'POST' && (PLAN_POSTS.has(p) || /^reframe\/[^/]+\/render$/.test(p))) return { type: 'plan' }
  return { type: 'forward' }
}
