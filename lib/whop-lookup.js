...
export async function whopPatch(path, apiKey, body) {
  return whopSend('PATCH', path, apiKey, body)
}

export async function retrievePlan(id, env) {
  const { apiKey } = whopConfig(env)
  const planId = String(id || '').trim()
  if (!apiKey || !planId) return null
  try {
    return await whopGet('/plans/' + encodeURIComponent(planId), apiKey)
  } catch {
    return null
  }
}
