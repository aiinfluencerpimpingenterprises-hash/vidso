...
/** True when the refresh token itself is dead — not a network blip. */
export function isInvalidRefreshError(err, status = err?.status) {
  const code = Number(status)
  if (code && code !== 400 && code !== 401) return false
  const msg = typeof err === 'string' ? err : String(err?.message || err?.error || err?.code || '')
  return /invalid.*(refresh|grant|token)|refresh token.*(expired|not found|revoked)|token not found/i.test(msg)
    || code === 401
}

/** Access JWTs can exceed a cookie's 4KB cap; localStorage stays the source of truth. */
export const COOKIE_MAX_ENCODED = 3500

export function cookiePayloadFits(value) {
  try {
    return encodeURIComponent(String(value || '')).length <= COOKIE_MAX_ENCODED
  } catch {
    return false
  }
}
...
function cookieSet(name, value, maxAge = MAX_AGE) {
  try {
    ...
    if (encoded.length > COOKIE_MAX_ENCODED) {
      document.cookie = name + '=; Path=/; Max-Age=0; SameSite=Lax' + domain + secure
      return
    }
...
export function hasSessionSecrets() {
