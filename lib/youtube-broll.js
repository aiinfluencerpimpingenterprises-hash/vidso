...
import { boundBrollQuery } from './broll-match.js'
...
export function brollQueriesFromScript(script = {}, opts = {}) {
  ...
  const topic = String(script.topic || '')
  const fromSections = sections
    .map((sec) => boundBrollQuery({
      query: sec.visual || sec.broll || sec.heading || sec.title || firstSentence(sec.text),
      heading: sec.heading || sec.title || '',
      text: sec.text || '',
      topic,
      stock: false,
    }))
    .map((q) => cleanBrollQuery(q))
    .filter(Boolean)
  const fromKeywords = (Array.isArray(script.keywords) ? script.keywords : [])
    .map((k) => boundBrollQuery({
      query: k,
      topic,
      text: sections.map((s) => s.text || s.heading || '').join(' '),
      stock: false,
    }))
  ...
  const fallback = cleanBrollQuery(script.topic)
