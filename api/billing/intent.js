...
function isPixelEvent(req) {
  try {
    return new URL(req.url || '/', 'http://localhost').searchParams.get('kind') === 'pixel'
  } catch {
    return false
  }
}
...
async function handlePixelEvent(req, res) {
...
export default async function handler(req, res) {
  ...
  if (isPixelEvent(req)) return handlePixelEvent(req, res)
  if (req.method !== 'POST') return send(res, 405, { error: 'Method not allowed' })
